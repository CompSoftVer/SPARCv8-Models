...
lemma swap_sub1_result: "snd (swap_sub1 instr rd s_val s) = False"    
  apply (simp add: swap_sub1_def)   
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)  
  apply (simp add: Let_def)       
  apply (simp add: case_prod_unfold)    
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)
  ...
  apply (simp add: Let_def) 
  apply (simp add: case_prod_unfold)  
  apply (simp add: write_reg_def simpler_modify_def raise_trap_def return_def)
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)     
  apply (simp add: get_curr_win_def simpler_gets_def return_def)    
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)  
  by (simp add: write_cpu_def simpler_modify_def)      
    
lemma swap_instr_result: "(fst i) \<in> {load_store_type SWAP, load_store_type SWAPA} \<Longrightarrow>
  snd (swap_instr i s) = False"   
  apply (simp add: swap_instr_def)
  apply (simp add: Let_def)
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)
  apply (simp add: raise_trap_def simpler_modify_def)
  apply (simp add: return_def)  
  by (simp add: swap_sub1_result)

lemma comp_swap_sub2_result: "snd (comp_swap_sub2 addr asi rs2 rd curr_win word0 s) = False"    
  apply (simp add: comp_swap_sub2_def)
  apply (simp add: simpler_gets_def bind_def h1_def h2_def) 
  apply (simp add: case_prod_unfold)     
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)  
  apply (simp add: Let_def)  
...
lemma comp_swap_sub1_result: "snd (comp_swap_sub1 instr rs1 rs2 rd s_val asi s) = False"
  apply (simp add: comp_swap_sub1_def)
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)  
  apply (simp add: Let_def)
  apply (simp add: case_prod_unfold)
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)  
  ...
  apply (simp add: Let_def)
  apply (simp add: case_prod_unfold)  
  apply (simp add: write_reg_def simpler_modify_def return_def raise_trap_def)    
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)    
  by (simp add: comp_swap_sub2_result)  

lemma comp_swap_instr_result: "(fst i) \<in> {load_store_type CASA, load_store_type CASXA} \<Longrightarrow>
  snd (comp_swap_instr i s) = False"     
  apply (simp add: comp_swap_instr_def)
  apply (simp add: Let_def)
  apply (simp add: simpler_gets_def bind_def h1_def h2_def)  
  apply (simp add: raise_trap_def simpler_modify_def)
  apply (simp add: return_def) 
  by (simp add: comp_swap_sub1_result)  
...
lemma supported_instr_set: "supported_instruction i = True \<Longrightarrow> 
  i \<in> {load_store_type LDSB,load_store_type LDUB,load_store_type LDUBA,
              ...
              bicc_type BN,
              load_store_type CASA, load_store_type CASXA}"
          ...
          next
            ...
            next 
              ...
              bicc_type BCS,bicc_type BLEU,bicc_type BCC,bicc_type BA}")
                                  ...
                                  next
                                    case False
                                    then have f18: "(fst i) \<in> {bicc_type BN, load_store_type CASA, load_store_type CASXA}" 
                                    using f17 by auto
                                  then show ?thesis using f1
                                  proof (cases "(fst i) = bicc_type BN")
                                    case True
                                    then show ?thesis using f1
                                    apply dispatch_instr_proof1
                                    apply auto
                                  ...
                                  next
                                    case False
                                    then have "(fst i) \<in> {load_store_type CASA, load_store_type CASXA}"  
                                      using f18 by auto
                                    then show ?thesis using f1
                                     apply dispatch_instr_proof1
                                    apply auto
                                    by (auto simp add: comp_swap_instr_result) 
                                  qed 
                                qed
                              qed
                            qed
      ...
      next
        ...
        next
          ...
          (fst v2) = ctrl_type RETT")
            ...
            qed
          next
            case False \<comment>\<open> Instruction is not RETT. \<close>
...
lemma comp_swap_sub2_privilege: 
assumes a1: "s' = snd (fst (comp_swap_sub2 address asi rs2 rd curr_win word0 
  (s::(('a::len0) sparc_state))))
  \<and> ((ucast (get_S (cpu_reg_val PSR s)))::word1) = 0"
shows "((ucast (get_S (cpu_reg_val PSR 
  (s'::(('a::len0) sparc_state)))))::word1) = 0"  
  using a1 apply (simp add: comp_swap_sub2_def)  
  apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
  apply (simp add: case_prod_unfold)
  apply (case_tac "fst (memory_read asi (address + 4) s) = None")  
   apply auto
  using get_curr_win_privilege raise_trap_privilege apply blast
  using raise_trap_privilege apply blast  
   apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)  
   apply (simp add: case_prod_unfold)
   apply (case_tac "word0 =
                            user_reg_val curr_win (rs2 AND 30)
                             (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))) \<and>
                            y = user_reg_val curr_win (rs2 OR 1)
                                 (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s)))")
    apply auto
     apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def) 
    apply (case_tac "memory_write asi address 15
                        (user_reg_val curr_win (rd AND 30) (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))))
                        (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))) =
                       None")
      apply auto
      apply (simp add: case_prod_unfold)
      apply (simp add: simpler_modify_def return_def)
  using raise_trap_privilege memory_read_privilege apply metis
     apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def) 
     apply (simp add: case_prod_unfold)
     apply (case_tac "memory_write asi (address + 4) 15
                             (user_reg_val curr_win (rd OR 1)
                               (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))))
                             (snd (fst (modify (\<lambda>s. y) (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s)))))) =
                            None")
      apply auto
        apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def)    
  using raise_trap_privilege memory_read_privilege memory_write_privilege apply metis    
       apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)    
     ...
     apply (simp add: case_prod_unfold)
     apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
    using memory_read_privilege memory_write_privilege write_reg_privilege
       apply metis
      apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
      apply (simp add: case_prod_unfold)
      apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
      using memory_read_privilege write_reg_privilege
        apply metis
       apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
       apply (simp add: case_prod_unfold)
       apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
      using memory_read_privilege write_reg_privilege apply metis                
      apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
      apply (simp add: case_prod_unfold)
      apply (case_tac "word0 =
                            user_reg_val curr_win (rs2 AND 30)
       ...
       apply auto
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
        apply (case_tac "memory_write asi address 15
                        (user_reg_val curr_win 0 (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))))
                        (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))) =
                       None")
         apply auto
         apply (simp add: case_prod_unfold return_def)
      ...
      using raise_trap_privilege memory_read_privilege apply metis
        ...
        apply (simp add: case_prod_unfold return_def)
        apply (case_tac "memory_write asi (address + 4) 15
                             (user_reg_val curr_win 1 (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s))))
                             (snd (fst (modify (\<lambda>s. y) (snd (fst (modify (\<lambda>sa. snd (memory_read asi (address + 4) s)) s)))))) =
                            None")
         apply auto
         apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
         apply (simp add: case_prod_unfold return_def)
         apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
      using raise_trap_privilege apply (metis memory_read_privilege memory_write_privilege) 
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
        apply (simp add: case_prod_unfold return_def)
        apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
      using memory_read_privilege write_reg_privilege memory_write_privilege apply metis
       apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
      using memory_read_privilege write_reg_privilege apply metis
      apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
      using memory_read_privilege write_reg_privilege by metis                    
  
lemma comp_swap_sub1_privilege: 
assumes a1: "s' = snd (fst (comp_swap_sub1 instr rs1 rs2 rd s_val asi 
  (s::(('a::len0) sparc_state))))
  \<and> ((ucast (get_S (cpu_reg_val PSR s)))::word1) = 0"
shows "((ucast (get_S (cpu_reg_val PSR 
  (s'::(('a::len0) sparc_state)))))::word1) = 0"  
proof (cases "fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s)))))::word2) \<noteq> 0")
  case True    
  then show ?thesis using a1
    apply (simp add: comp_swap_sub1_def)
    apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
...
next
  case False
  then have f1: "\<not> (fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s)))))::word2) \<noteq> 0)"
    by auto      
  then show ?thesis using a1
    proof (cases "fst (memory_read asi (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s))))
      ...
      then show ?thesis using a1
        ...
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)          
        apply (simp add: case_prod_unfold)          
        using f1 apply clarsimp \<comment>\<open> This shows that isabelle is really dumb. \<close>          
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)          
        apply (simp add: case_prod_unfold return_def)  
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)  
        apply (simp add: case_prod_unfold) 
    ...
    next
      case False
      then have f2: "\<not> (fst (memory_read asi (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s))))
                             (snd (fst (get_curr_win () s)))) =
                       None)"  by auto
      then show ?thesis using a1
        apply (simp add: comp_swap_sub1_def)
        ...
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
        apply (simp add: case_prod_unfold) 
        apply (case_tac "fst instr = load_store_type CASA")
         apply clarsimp
         apply (case_tac "y = user_reg_val (fst (fst (get_curr_win () s))) rs2
                                 (snd (fst (modify (\<lambda>sa. snd (memory_read asi
                                                                (user_reg_val (fst (fst (get_curr_win () s))) rs1
 (snd (fst (get_curr_win () s))))
                                                                (snd (fst (get_curr_win () s)))))
                                             (snd (fst (get_curr_win () s))))))")
          apply clarsimp
          apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def)
          apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
          apply (case_tac "memory_write asi (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s)))) 15
                        (user_reg_val (fst (fst (get_curr_win () s))) rd
                          (snd (memory_read asi
                                 (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s))))
                                 (snd (fst (get_curr_win () s))))))
                        (snd (memory_read asi (user_reg_val (fst (fst (get_curr_win () s))) rs1 (snd (fst (get_curr_win () s))))
                               (snd (fst (get_curr_win () s))))) =
                       None")
           apply clarsimp
        using raise_trap_privilege memory_read_privilege apply (metis get_curr_win_privilege option.simps(3))  
          apply clarsimp
          apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)  
          apply (simp add: case_prod_unfold)
          apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def) 
          apply (case_tac "rd \<noteq> 0")
           apply clarsimp
        using memory_read_privilege get_curr_win_privilege memory_write_privilege write_reg_privilege            
           apply (metis (no_types, hide_lams))
          apply clarsimp
          apply (meson get_curr_win_privilege memory_read_privilege memory_write_privilege)
         apply clarsimp
         apply (case_tac "rd \<noteq> 0")
          apply clarsimp
          apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def)
        using memory_read_privilege get_curr_win_privilege  write_reg_privilege apply metis
         apply clarsimp
         apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def)
        using memory_read_privilege get_curr_win_privilege apply metis 
        apply clarsimp
        apply (simp add: simpler_modify_def simpler_gets_def bind_def return_def)  
        apply auto          
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        ...
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege apply metis
        using comp_swap_sub2_privilege get_curr_win_privilege memory_read_privilege by metis
    qed              
qed
  
lemma comp_swap_instr_privilege: 
assumes a1: "s' = snd (fst (comp_swap_instr instr 
  (s::(('a::len0) sparc_state))))
  \<and> ((ucast (get_S (cpu_reg_val PSR s)))::word1) = 0"
shows "((ucast (get_S (cpu_reg_val PSR 
  (s'::(('a::len0) sparc_state)))))::word1) = 0"  
  ...
  apply (simp add: Let_def)  
  apply (simp add: simpler_gets_def bind_def h1_def h2_def) 
  using raise_trap_privilege comp_swap_sub1_privilege
  by (metis (no_types, hide_lams) fst_conv prod.case_eq_if return_def snd_conv)   
...
shows "((ucast (get_S (bitOR (bitAND w1 
  ...
  by (metis (no_types) assms ucast_s ucast_s2 word_ao_absorbs(8) word_bool_alg.conj_disj_distrib2)
                              ...
                              next
                                ...
                                next
                                  ...
                                  next
                                    ...
                                    next
                                      case False
                                      then have f19: "\<not>(fst instr \<in> {bicc_type BE,bicc_type BNE,
                                      bicc_type BGU,bicc_type BLE,bicc_type BL,bicc_type BGE,
                                      bicc_type BNEG,bicc_type BG,bicc_type BCS,bicc_type BLEU,
                                      bicc_type BCC,bicc_type BA,bicc_type BN})" by auto                                      
                                      then show ?thesis 
                                      proof (cases "fst instr \<in> {load_store_type CASA, load_store_type CASXA}")
                                        case True
                                        then show ?thesis using a1 f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 
                                      f12 f13 f14 f15 f16 f17 f18 f19
                                      apply dispath_instr_privilege_proof
                                      by (blast intro: comp_swap_instr_privilege)
                                      next
                                        case False
                                        then show ?thesis using a1 f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 
                                      f12 f13 f14 f15 f16 f17 f18 f19
...
definition user_accessible_pte:: "PTE_flags \<Rightarrow> bool" where
"user_accessible_pte pte \<equiv> mmu_readable (get_acc_flag pte) 10" 
\<comment>\<open> Passing asi = 8 is the same. \<close>

definition user_writable_pte:: "PTE_flags \<Rightarrow> bool" where
"user_writable_pte pte \<equiv> mmu_writable (get_acc_flag pte) 10" 
\<comment>\<open> Passing asi = 8 is the same. \<close>

lemma mmu_writable_10_8: 
assumes a1: "mmu_writable (get_acc_flag pte) 10"
shows "mmu_writable (get_acc_flag pte) 8"
  using a1 by (simp add: mmu_writable_def)

lemma mmu_writable_8_10: 
assumes a1: "mmu_writable (get_acc_flag pte) 8"
shows "mmu_writable (get_acc_flag pte) 10"
using a1 by (simp add: mmu_writable_def)

lemma mmu_readable_8_10: 
...
lemma mmu_readable_10_8: 
assumes a1: "mmu_readable (get_acc_flag (snd p)) 10"
shows "mmu_readable (get_acc_flag (snd p)) 8"
  using a1 by (simp add: mmu_readable_def)

lemma mmu_writable_readable: "mmu_writable (get_acc_flag pte) 10 \<Longrightarrow> 
  mmu_readable (get_acc_flag pte) 10"
  apply (auto simp add: mmu_writable_def mmu_readable_def)
  by metis

definition mem_equal:: "('a) sparc_state \<Rightarrow> ('a) sparc_state \<Rightarrow> 
  phys_address \<Rightarrow> bool" where
"mem_equal s1 s2 pa \<equiv> \<forall> asi i. (asi \<in> {8,9,10,11} \<and> i \<in> {0,1,2,3}) \<longrightarrow>
  (mem s1) asi ((pa AND 68719476732) + i) = (mem s2) asi ((pa AND 68719476732) + i)"

lemma mem_equal: "mem_equal s1 s2 pa =
...
lemma mem_equal_com: "mem_equal s1 s2 pa \<Longrightarrow> mem_equal s2 s1 pa"
...
"low_equal s1 s2 \<equiv> 
  ...
  (dwrite s1) = (dwrite s2) \<and>
  (\<forall>va.(virt_to_phys va (mmu s2) (mem s2) = None \<and> virt_to_phys va (mmu s1) (mem s1) = None) \<or> 
       (\<exists> pa1 pte1 pa2 pte2. 
         (virt_to_phys va (mmu s1) (mem s1) = Some (pa1, pte1)) \<and>
         (virt_to_phys va (mmu s2) (mem s2) = Some (pa2, pte2)) \<and>
         ((\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2)) \<or>
          ((user_writable_pte pte1) \<and> (user_writable_pte pte2))) \<and>
         ((\<not>(user_accessible_pte pte1) \<and> \<not>(user_accessible_pte pte2)) \<or>
          (((user_accessible_pte pte1) \<and> (user_accessible_pte pte2) \<and> 
            pa1 = pa2 \<and> (mem_equal s1 s2 pa1))))
       )) \<and>
  (mmu s1) = (mmu s2) \<and> 
...
(s\<lparr>cpu_reg := new_cpu_reg,
  ...
  apply (simp add: mem_val_alt_def Let_def)
  apply auto
    apply (simp add: mem_val_alt_89_def Let_def)
   apply (simp add: mem_val_alt_89_def Let_def)
  apply (simp add: mem_val_alt_1011_def Let_def)
  apply auto
     apply (simp add: mem_val_alt_89_def Let_def)
  apply (simp add: mem_val_alt_89_def Let_def)
  apply (simp add: mem_val_alt_89_def Let_def)
  by (simp add: mem_val_alt_89_def Let_def)
...
low_equal t1 t2"
  apply (simp add: low_equal_def)
  apply auto
     apply (simp add: user_accessible_pte_def mem_equal)
     apply auto 
     apply (simp add: user_accessible_pte_def mem_equal)
    apply auto
   apply (simp add: user_accessible_pte_def mem_equal)
...
lemma mod_state_low_equal2: 
  assumes a1: "low_equal s1 s2"
  and a2: "t1 = (s1\<lparr>cpu_reg := new_cpu_reg1,
   user_reg := new_user_reg1,
   dwrite := new_dwrite1,
   state_var := new_state_var1,
   traps := new_traps1,
   undef := new_undef1\<rparr>)"
  and a3: "t2 = (s2\<lparr>cpu_reg := new_cpu_reg2,
   user_reg := new_user_reg2,
   dwrite := new_dwrite2,
   state_var := new_state_var2,
   traps := new_traps2,
   undef := new_undef2\<rparr>)"
  and a4: "new_cpu_reg1 = new_cpu_reg2 \<and> new_user_reg1 = new_user_reg2 \<and> new_dwrite1 = new_dwrite2
          \<and> new_state_var1 = new_state_var2 \<and> new_traps1 = new_traps2 \<and> new_undef1 = new_undef2"
shows "low_equal t1 t2"
proof -
  show ?thesis using a1 a2 a3 a4 mod_state_low_equal
    by blast 
qed

lemma user_reg_state_mod_low_equal: 
assumes a1: "low_equal s1 s2"           
and a2: "t1 = (s1\<lparr>user_reg := new_user_reg\<rparr>)"
and a3: "t2 = (s2\<lparr>user_reg := new_user_reg\<rparr>)"
shows "low_equal t1 t2"
proof -
  from a1 have f1: "cpu_reg s1 = cpu_reg s2"
    by (simp add: low_equal_def)
  from a1 have f2: "dwrite s1 = dwrite s2"
    by (simp add: low_equal_def)
  from a1 have f3: "state_var s1 = state_var s2"
    by (simp add: low_equal_def)
  from a1 have f4: "traps s1 = traps s2"
    by (simp add: low_equal_def)
  from a1 have f5: "undef s1 = undef s2"
    by (simp add: low_equal_def)
  have f6: "low_equal s1 s2 \<and>
        t1 = (s1\<lparr>cpu_reg := (cpu_reg s2),
           user_reg := new_user_reg,
           dwrite := (dwrite s2),
           state_var := (state_var s2),
           traps := (traps s2),
           undef := (undef s2)\<rparr>) \<and>
        ...
        low_equal t1 t2"
    using mod_state_low_equal2 by blast 
  then have "low_equal s1 s2 \<and>
...
assumes a1: "low_equal s1 s2"           
and a2: "t1 = (s1\<lparr>traps := new_traps\<rparr>)"
and a3: "t2 = (s2\<lparr>traps := new_traps\<rparr>)"
shows "low_equal t1 t2"
proof -
  from a1 have f1: "cpu_reg s1 = cpu_reg s2"
    by (simp add: low_equal_def)
  ...
  from a1 have f3: "state_var s1 = state_var s2"
    by (simp add: low_equal_def)
  from a1 have f4: "user_reg s1 = user_reg s2"
    by (simp add: low_equal_def)
  from a1 have f5: "undef s1 = undef s2"
    by (simp add: low_equal_def)
  have f6: "low_equal s1 s2 \<and>
...
proof -
  ...
  from a1 have f2: "dwrite s1 = dwrite s2"
    by (simp add: low_equal_def)
  from a1 have f3: "traps s1 = traps s2"
    by (simp add: low_equal_def)
  from a1 have f4: "user_reg s1 = user_reg s2"
    by (simp add: low_equal_def)
  from a1 have f5: "undef s1 = undef s2"
...
apply (simp add: mem_val_alt_def mem_val_alt_89_def mem_val_alt_1011_def)
  apply (simp add: Let_def)
  apply auto
     apply (simp add: mem_val_alt_89_def Let_def)
    apply (simp add: mem_val_alt_89_def Let_def)
...
apply (case_tac "virt_to_phys address (mmu s2) (mem s2) = None")
  apply auto
     apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
  apply auto
     apply (metis (no_types) option.distinct(1))
    apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
     apply auto
    apply (metis (no_types) option.distinct(1))
   apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
    apply auto
     apply (metis (no_types) option.distinct(1))
     using mem_val_w32_10_mem_equal
         apply (metis fst_conv option.distinct(1) option.inject snd_conv user_accessible_pte_def)
     apply (metis (mono_tags, hide_lams) option.inject snd_conv user_accessible_pte_def)
       apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
        apply auto
       apply (metis (mono_tags, hide_lams) option.inject snd_conv user_accessible_pte_def)
      apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
       apply auto
        ...
        apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
         apply auto
        by (metis (mono_tags, hide_lams) option.inject snd_conv user_accessible_pte_def)
   ...
   apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
    apply auto
     ...
     using mem_val_w32_8_mem_equal
         apply (metis (mono_tags) fst_conv mmu_readable_8_10 option.distinct(1) 
                option.inject snd_conv user_accessible_pte_def)
        apply (metis (mono_tags) mmu_readable_10_8 mmu_readable_8_10 option.inject 
               snd_conv user_accessible_pte_def) 
       apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
        apply auto
        apply (metis (mono_tags) mmu_readable_10_8 mmu_readable_8_10 option.inject 
               snd_conv user_accessible_pte_def) 
...
apply (metis (mono_tags) mmu_readable_10_8 mmu_readable_8_10 option.inject 
     ...
     apply (case_tac "virt_to_phys address (mmu s2) (mem s1) = None")
      apply auto
     by (metis (mono_tags) mmu_readable_10_8 mmu_readable_8_10 option.inject 
               snd_conv user_accessible_pte_def) 
...
virt_to_phys addr (mmu s1) (mem s1) = Some (pa1, pte1) \<Longrightarrow>
virt_to_phys addr (mmu s2) (mem s2) = Some (pa2, pte2) \<Longrightarrow> 
user_accessible_pte pte1 \<Longrightarrow> user_accessible_pte pte2 \<Longrightarrow> 
mem_equal s1 s2 pa2"
  apply (auto simp add: low_equal_def)
  apply (metis (mono_tags, hide_lams) fst_conv not_None_eq option.inject snd_conv)
  by (metis fst_conv option.distinct(1) option.sel snd_conv)
...
lemma store_word_mem_low_equal_none:
  assumes a1: "low_equal s1 s2"
      and a2: "store_word_mem (add_data_cache s1 addr data bm) addr data bm 10 = None"
    shows "store_word_mem (add_data_cache s2 addr data bm) addr data bm 10 = None"
proof -
  let ?t1 = "add_data_cache s1 addr data bm"
  let ?t2 = "add_data_cache s2 addr data bm"
  from a1 have f1: "low_equal ?t1 ?t2"
    using add_data_cache_low_equal 
    by blast
  then have f2: "virt_to_phys addr (mmu ?t1) (mem ?t1) = None \<Longrightarrow> 
                 virt_to_phys addr (mmu ?t2) (mem ?t2) = None"
    apply (simp add: low_equal_def)
    by auto
  from a1 a2 show ?thesis 
    apply (simp add: store_word_mem_def)
    apply (cases "virt_to_phys addr (mmu ?t1) (mem ?t1) = None")
     apply auto
     apply (cases "virt_to_phys addr (mmu ?t2) (mem ?t2) = None")
    using f2 apply auto
    apply (cases "virt_to_phys addr (mmu ?t2) (mem ?t2) = None")
     apply auto
    apply (simp add: low_equal_def)
    apply auto
     apply (simp add: user_accessible_pte_def)
    using f1 unfolding low_equal_def
    apply (metis option.distinct(1) option.sel prod.inject user_writable_pte_def) 
    using f1 unfolding low_equal_def
    by (metis option.distinct(1) option.sel prod.inject user_writable_pte_def) 
qed
...
apply (simp add: mem_val_alt_def mem_val_alt_89_def mem_val_alt_1011_def)
apply (simp add: Let_def)
using a1 apply (simp add: mem_equal)
...
proof (cases "virt_to_phys addra (mmu s1) ((mem s1)(10 := mem s1 10(addr \<mapsto> val), 
  ...
  then have "virt_to_phys addra (mmu s1) ((mem s1)(10 := mem s1 10(addr \<mapsto> val), 
    ...
    using a1 apply (simp add: low_equal_def)
    by (metis option.distinct(1) virt_to_phys_unchanged2) 
  then show ?thesis
  by (simp add: load_word_mem_def)
next
  case False
  then obtain pa1 pte1 pa2 pte2 where f1:
      "virt_to_phys addra (mmu s1) ((mem s1)(10 := mem s1 10(addr \<mapsto> val), 
          11 := (mem s1 11)(addr := None))) = Some (pa1, pte1) \<and>
             virt_to_phys addra (mmu s2) ((mem s2)(10 := mem s2 10(addr \<mapsto> val), 
          11 := (mem s2 11)(addr := None))) = Some (pa2, pte2)"
    using a1 apply (auto simp add: mmu_low_equal)
    using a1 apply (simp add: low_equal_def)
    by (metis virt_to_phys_unchanged2) 
  then have f2: "virt_to_phys addra (mmu s1) (mem s1) = Some (pa1,pte1) \<and>
                 virt_to_phys addra (mmu s2) (mem s2) = Some (pa2,pte2)"
    using virt_to_phys_unchanged2 by metis
  then have f3: "(user_writable_pte pte1 \<and> user_writable_pte pte2) \<or>
                 (\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2))"
    using a1 unfolding low_equal_def
    by (metis option.sel snd_conv) 
  also have f4: "(user_accessible_pte pte1 \<and> user_accessible_pte pte2 \<and> pa1 = pa2 \<and> 
                 mem_equal s1 s2 pa1) \<or>
                 (\<not>(user_accessible_pte pte1) \<and> \<not>(user_accessible_pte pte2))"
    using a1 unfolding low_equal_def
    by (metis False f1 f2 option.inject prod.sel(1) snd_conv)
  then have f5: "(user_accessible_pte pte1 \<and> user_accessible_pte pte2) \<Longrightarrow> 
                 pa1 = pa2 \<and> mem_equal s1 s2 pa1"
    by auto
  then have f6: "(user_accessible_pte pte1 \<and> user_accessible_pte pte2) \<Longrightarrow> pa1 = pa2 \<and>
           mem_val_w32 8 pa1
            (s1\<lparr>mem := (mem s1)(10 := mem s1 10(addr \<mapsto> val), 11 := (mem s1 11)(addr := None))\<rparr>) =
           mem_val_w32 8 pa1
            (s2\<lparr>mem := (mem s2)(10 := mem s2 10(addr \<mapsto> val), 11 := (mem s2 11)(addr := None))\<rparr>)"
    using mem_val_w32_8_unchanged
    by auto
  then show ?thesis using f1 f2 a1  
  apply (simp add: load_word_mem_def)
  apply auto
    apply (simp add: low_equal_def)
    apply (cases "virt_to_phys addra (mmu s2) (mem s1) = None")
     apply auto
    using mem_val_w32_8_unchanged mmu_readable_8_10 unfolding user_accessible_pte_def
       apply auto
    using f4 mmu_readable_10_8 user_accessible_pte_def by auto
qed
...
"mem_val_alt 10 addr (s\<lparr>mem := (mem s)(10 := mem s 10(addr \<mapsto> val), 
  ...
  by (simp add: mem_val_alt_1011_def Let_def)
...
apply (simp add: mem_val_alt_def)
  ...
  apply auto               
                 apply (simp add: mem_val_alt_89_def Let_def)
                 apply auto
  using a1 unfolding mem_equal by auto
...
virt_to_phys addra (mmu s1) (mem s1) = Some (pa1, pte1) \<Longrightarrow>
virt_to_phys addra (mmu s2) (mem s2) = Some (pa2, pte2) \<Longrightarrow>
mmu_readable (get_acc_flag pte1) 10 \<Longrightarrow>
pa1 = pa2 \<and> mem_equal s1 s2 pa1"
apply (simp add: low_equal_def)
  apply (simp add: user_accessible_pte_def)
  by (metis option.distinct(1) option.sel prod.sel(1) snd_conv)
...
proof (cases "virt_to_phys addra (mmu s1) ((mem s1)(10 := mem s1 10(addr \<mapsto> val), 
  ...
  then have "virt_to_phys addra (mmu s1) ((mem s1)(10 := mem s1 10(addr \<mapsto> val), 
          ...
          11 := (mem s2 11)(addr := None))) = None"
    using a1 apply (auto simp add: mmu_low_equal)
    using a1 virt_to_phys_unchanged2
  proof -
    assume a1: "virt_to_phys addra (mmu s2) ((mem s1) (10 := mem s1 10(addr \<mapsto> val), 11 := (mem s1 11)(addr := None))) = None"
    have "\<forall>w s sa. \<exists>wa wb. \<forall>sb sc. (\<not> low_equal (sb::'a sparc_state) sc \<or> mmu sb = mmu sc) \<and> (\<not> low_equal (s::'a sparc_state) sa \<or> virt_to_phys w (mmu sa) (mem sa) = None \<or> virt_to_phys w (mmu s) (mem s) = Some (wa, wb))"
      using low_equal_def by blast
    then show "virt_to_phys addra (mmu s2) ((mem s2) (10 := mem s2 10(addr \<mapsto> val), 11 := (mem s2 11)(addr := None))) = None"
      using a1 by (metis (no_types) assms option.distinct(1) virt_to_phys_unchanged2)
  qed 
  then show ?thesis
  by (simp add: load_word_mem_def)
next
  case False
  then obtain pa1 pte1 where f1:
          "virt_to_phys addra (mmu s1) ((mem s1)(10 := mem s1 10(addr \<mapsto> val), 
          11 := (mem s1 11)(addr := None))) = Some (pa1,pte1)"
    by auto
  then have f2: "virt_to_phys addra (mmu s1) (mem s1) = Some (pa1,pte1)"
    using virt_to_phys_unchanged2
    by metis 
  then obtain pa2 pte2 where f3: "virt_to_phys addra (mmu s2) (mem s2) = Some (pa2,pte2)"
    using a1 unfolding low_equal_def
    by metis 
  then have f4: "virt_to_phys addra (mmu s2) ((mem s2)(10 := mem s2 10(addr \<mapsto> val), 
          11 := (mem s2 11)(addr := None))) = Some (pa2,pte2)"
    using virt_to_phys_unchanged2
    by metis
  from a1 f2 f3 have f5: "user_accessible_pte pte1 = user_accessible_pte pte2"
    unfolding low_equal_def
    by (metis option.inject snd_conv)
  then show ?thesis 
  proof (cases "user_accessible_pte pte1")
    case True
    then have f6: "(user_accessible_pte pte1) \<and> (user_accessible_pte pte2)"
      using f5 by simp
    then have f7: "pa1 = pa2 \<and> mem_equal s1 s2 pa1"
      using a1 f2 f3 unfolding low_equal_def
      by (metis False f1 fst_conv option.inject snd_conv) 
    then show ?thesis 
      unfolding load_word_mem_def user_accessible_pte_def 
      apply (auto simp add: f1 f4)
      using mem_val_w32_10_unchanged
        apply blast 
      using f6 unfolding user_accessible_pte_def by auto
  next
    case False
    then have "\<not>(user_accessible_pte pte1) \<and> \<not>(user_accessible_pte pte2)"
      using f5 by simp
    then show ?thesis 
      unfolding load_word_mem_def user_accessible_pte_def 
      by (auto simp add: Let_def f1 f4)
  qed
qed
...
assumes a1: "low_equal s1 s2"
and a2: "t1 = s1\<lparr>mem := (mem s1)(10 := mem s1 10(addr \<mapsto> val), 11 := (mem s1 11)(addr := None))\<rparr>"
and a3: "t2 = s2\<lparr>mem := (mem s2)(10 := mem s2 10(addr \<mapsto> val), 11 := (mem s2 11)(addr := None))\<rparr>"
shows "low_equal t1 t2"
proof -
  have f14: "\<forall>va. (virt_to_phys va (mmu t1) (mem t1) = None \<and> 
    virt_to_phys va (mmu t2) (mem t2) = None) \<or>
    (\<exists>pa1' pa2' pte1' pte2'. (virt_to_phys va (mmu t2) (mem t2) = Some (pa2',pte2') \<and> 
    virt_to_phys va (mmu t1) (mem t1) = Some (pa1',pte1')) \<and>
    ((\<not>(user_writable_pte pte1') \<and> \<not>(user_writable_pte pte2')) \<or>
          ((user_writable_pte pte1') \<and> (user_writable_pte pte2'))) \<and>
         ((\<not>(user_accessible_pte pte1') \<and> \<not>(user_accessible_pte pte2')) \<or>
          (((user_accessible_pte pte1') \<and> (user_accessible_pte pte2') \<and> 
            pa1' = pa2' \<and> (mem_equal t1 t2 pa1')))))"
  proof -
    {  fix va 
  have f1: "virt_to_phys va (mmu s2) (mem s2) = None \<Longrightarrow> virt_to_phys va (mmu s1) (mem s1) = None"
    using a1 unfolding low_equal_def by auto
  also have f2: "virt_to_phys va (mmu s1) (mem s1) = None \<Longrightarrow> 
    virt_to_phys va (mmu s2) (mem s2) = None"
    using a1 unfolding low_equal_def by auto
  have f3: "mmu s1 = mmu t1 \<and> mmu s2 = mmu t2" 
    using a2 a3 by auto
  have f3': "mem t2 = (mem s2)(10 := mem s2 10(addr \<mapsto> val), 11 := (mem s2 11)(addr := None))"
    using a3 by auto
  then have f4: "virt_to_phys va (mmu s2) (mem s2) = None \<Longrightarrow> 
    virt_to_phys va (mmu t2) (mem t2) = None"
    using a3 f3 virt_to_phys_unchanged2
    by metis 
  have f4': "mem t1 = (mem s1)(10 := mem s1 10(addr \<mapsto> val), 11 := (mem s1 11)(addr := None))"
    using a2 by auto
  then have f5: "virt_to_phys va (mmu s2) (mem s2) = None \<Longrightarrow> 
    virt_to_phys va (mmu t1) (mem t1) = None"
    using a2 f3 f1 virt_to_phys_unchanged2
    by metis
  obtain pa2 pte2 where f6: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
    virt_to_phys va (mmu s2) (mem s2) = Some (pa2,pte2)"
    by auto
  obtain pa1 pte1 where f7: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
    virt_to_phys va (mmu s1) (mem s1) = Some (pa1,pte1)"
    using f6 a1 unfolding low_equal_def
    by blast 
  from f6 f7 have f8: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
       ((\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2)) \<or>
          ((user_writable_pte pte1) \<and> (user_writable_pte pte2))) \<and>
         ((\<not>(user_accessible_pte pte1) \<and> \<not>(user_accessible_pte pte2)) \<or>
          (((user_accessible_pte pte1) \<and> (user_accessible_pte pte2) \<and> 
  ...
  then have f9: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
       ((\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2)) \<or>
          ((user_writable_pte pte1) \<and> (user_writable_pte pte2))) \<and>
         ((\<not>(user_accessible_pte pte1) \<and> \<not>(user_accessible_pte pte2)) \<or>
          (((user_accessible_pte pte1) \<and> (user_accessible_pte pte2) \<and> 
            pa1 = pa2 \<and> (mem_equal t1 t2 pa1))))"
    using mem_equal_unchanged
    using a2 a3 by blast 
  from f6 have f10: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
    virt_to_phys va (mmu t2) (mem t2) = Some (pa2,pte2)"
    using f3 f3' virt_to_phys_unchanged2
    by metis
  from f7 have f11: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
    virt_to_phys va (mmu t1) (mem t1) = Some (pa1,pte1)"
    using f3 f4' virt_to_phys_unchanged2
    by metis
  then have f12: "virt_to_phys va (mmu s2) (mem s2) \<noteq> None \<Longrightarrow> 
    (virt_to_phys va (mmu t2) (mem t2) = Some (pa2,pte2) \<and> 
    virt_to_phys va (mmu t1) (mem t1) = Some (pa1,pte1)) \<and>
    ((\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2)) \<or>
          ((user_writable_pte pte1) \<and> (user_writable_pte pte2))) \<and>
         ((\<not>(user_accessible_pte pte1) \<and> \<not>(user_accessible_pte pte2)) \<or>
          (((user_accessible_pte pte1) \<and> (user_accessible_pte pte2) \<and> 
            pa1 = pa2 \<and> (mem_equal t1 t2 pa1))))"
    using f10 f9 by auto
  then have f13: "(virt_to_phys va (mmu t1) (mem t1) = None \<and> 
    virt_to_phys va (mmu t2) (mem t2) = None) \<or>
    ((virt_to_phys va (mmu t2) (mem t2) = Some (pa2,pte2) \<and> 
    virt_to_phys va (mmu t1) (mem t1) = Some (pa1,pte1)) \<and>
    ((\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2)) \<or>
    ...
    using f4 f5
    by blast 
  then have f14: "(virt_to_phys va (mmu t1) (mem t1) = None \<and> 
    virt_to_phys va (mmu t2) (mem t2) = None) \<or>
    (\<exists> pa1 pa2 pte1 pte2. (virt_to_phys va (mmu t2) (mem t2) = Some (pa2,pte2) \<and> 
    virt_to_phys va (mmu t1) (mem t1) = Some (pa1,pte1)) \<and>
    ((\<not>(user_writable_pte pte1) \<and> \<not>(user_writable_pte pte2)) \<or>
  ...
  then show ?thesis 
    by auto
  qed
  then show ?thesis using a1 a2 a3 unfolding low_equal_def
    by auto
qed
...
assumes a1: "low_equal s1 s2"
and a2: "t1 = mem_mod_w32 10 aa bm data s1"
and a3: "t2 = mem_mod_w32 10 a bm data s2"
and a4: "virt_to_phys addr (mmu s2) (mem s2) = Some (a, b)"
and a5: "mmu_writable (get_acc_flag b) 10"
and a6: "virt_to_phys addr (mmu s1) (mem s1) = Some (aa, ba)"
and a7: "mmu_writable (get_acc_flag ba) 10"
shows "low_equal t1 t2"
proof -
  have f1: "aa = a"
    using a1 a4 a5 a6 a7 unfolding low_equal_def
    by (metis mmu_writable_readable option.sel prod.inject user_accessible_pte_def)
  then show ?thesis using a1 a2 a3
    apply (simp add: mem_mod_w32_def)
    apply (simp add: Let_def)
...
apply (case_tac "mmu_writable (get_acc_flag b) 10")
   apply auto
  apply (cases "virt_to_phys addr (mmu s1) (mem s1) = None")
   apply auto
  apply (case_tac "mmu_writable (get_acc_flag ba) 10")
   apply auto
using mem_mod_w32_low_equal by metis
...
lemma comp_swap_sub2_low_equal: 
assumes a1: "low_equal s1 s2 \<and>
(fst instr = load_store_type CASA \<or>
fst instr = load_store_type CASXA) \<and>
t1 = snd (fst (comp_swap_sub2 address 10 rs2 rd curr_win word0 s1)) \<and> 
t2 = snd (fst (comp_swap_sub2 address 10 rs2 rd curr_win word0 s2))" 
shows "low_equal t1 t2" 
proof (cases "fst (memory_read 10 (address + 4) s1) = None")
  case True
  then have f1: "fst (memory_read 10 (address + 4) s1) = None"  
    by auto
  then have f2: "fst (memory_read 10 (address + 4) s2) = None"
    using mem_read_low_equal a1 by fastforce
  then show ?thesis using a1
    apply (simp add: comp_swap_sub2_def)
    apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
    using f1 f2 apply (simp add: case_prod_unfold)
    apply (simp add: raise_trap_def add_trap_set_def)
    apply (simp add: simpler_modify_def)      
    using mod_trap_low_equal traps_low_equal by metis       
next
  case False
  then have f3: "fst (memory_read 10 (address + 4) s1) \<noteq> None" 
    by auto
  then have f4: "fst (memory_read 10 (address + 4) s2) \<noteq> None"
    using mem_read_low_equal a1 by fastforce   
  have f0: "low_equal s1 s2  \<Longrightarrow> \<forall>v1 v2. (memory_write 10 address 15
    (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)))
    (snd (memory_read 10 (address + 4) s1)) = Some v1 \<and>
    memory_write 10 address 15
    (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2)))
    (snd (memory_read 10 (address + 4) s2)) = Some v2) \<longrightarrow> low_equal v1 v2"
    apply auto    
    using mem_read2_low_equal user_reg_val_low_equal memory_write_low_equal
    proof -
      fix v1 :: "'a sparc_state" and v2 :: "'a sparc_state"
      assume a1: "low_equal s1 s2"
      assume a2: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2))) (snd (memory_read 10 (address + 4) s2)) = Some v2"
      assume a3: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1))) (snd (memory_read 10 (address + 4) s1)) = Some v1"
      have "\<And>w wa wb. user_reg_val w wa (snd (memory_read 10 wb s2)) = user_reg_val w wa (snd (memory_read 10 wb s1))"
        using a1 by (metis (no_types) mem_read2_low_equal user_reg_val_low_equal)
      then have "\<And>s. Some s \<noteq> Some v2 \<or> \<not> low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2)) \<or> low_equal v1 s"
        using a3 a2 by (metis memory_write_low_equal)
      then show "low_equal v1 v2"
        using a1 by (metis mem_read2_low_equal)
    qed     
  then show ?thesis
    proof (cases "word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1)) \<and>
                  fst (memory_read 10 (address + 4) s1) = Some (user_reg_val curr_win (rs2 OR 1)
                                     (snd (memory_read 10 (address + 4) s1)))")
      case True
      then have f5: "word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1)) \<and>
                     fst (memory_read 10 (address + 4) s1) = Some (user_reg_val curr_win (rs2 OR 1)
                                     (snd (memory_read 10 (address + 4) s1)))"
        by auto
      then have f6: "word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s2)) \<and>
                     fst (memory_read 10 (address + 4) s2) = Some (user_reg_val curr_win (rs2 OR 1)
                                     (snd (memory_read 10 (address + 4) s2)))"
        using mem_read_low_equal mem_read2_low_equal user_reg_val_low_equal a1
      proof -
        have f1: "low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2))"
          using a1 mem_read2_low_equal by blast
        have "fst (memory_read 10 (address + 4) s1) = fst (memory_read 10 (address + 4) s2)"
          using a1 mem_read_low_equal by blast
        then show ?thesis
          using f1 by (simp add: True user_reg_val_low_equal)
      qed 
      then show ?thesis 
        proof (cases "memory_write 10 address 15
                      (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)))
                      (snd (memory_read 10 (address + 4) s1)) = None")
          case True
          then have f7: "memory_write 10 address 15
                      (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)))
                      (snd (memory_read 10 (address + 4) s1)) = None"  
            by auto
          then have f8: "memory_write 10 address 15
                      (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2)))
                      (snd (memory_read 10 (address + 4) s2)) = None"
            using mem_read2_low_equal user_reg_val_low_equal memory_write_low_equal memory_write_low_equal_none a1
          proof -
            have f1: "low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2))"
              using a1 mem_read2_low_equal by blast
            have "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
              by (metis user_reg_val_low_equal)
            then show ?thesis
              using f1 f7 memory_write_low_equal_none by fastforce
          qed
          have "t1 = snd (memory_read 10 (address + 4) s1)
            \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s1)))\<rparr> \<and> 
                  t2 = snd (memory_read 10 (address + 4) s2)
            \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s2)))\<rparr>"
            using a1 
            apply (simp add: comp_swap_sub2_def)                
            apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)          
            using f3 f4 apply (simp add: case_prod_unfold)          
            apply clarsimp                
            apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)  
            apply (simp add: case_prod_unfold)
            using f5 f6 apply (simp add: simpler_modify_def)
            using f7 f8 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
            apply (simp add: case_prod_unfold return_def)
            apply (simp add: raise_trap_def add_trap_set_def)
            apply (simp add: simpler_modify_def)
            by fastforce              
          then show ?thesis using mod_trap_low_equal traps_low_equal mem_read2_low_equal a1
          proof -
            have f1: "\<And>s. \<not> low_equal (snd (memory_read 10 (address + 4) s1)) s \<or> low_equal t1 (s\<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s1)))\<rparr>)"
              using \<open>t1 = snd (memory_read 10 (address + 4) s1) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s1)))\<rparr> \<and> t2 = snd (memory_read 10 (address + 4) s2) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s2)))\<rparr>\<close> mod_trap_low_equal by blast
            have "\<And>w. traps (snd (memory_read 10 w s2)) = traps (snd (memory_read 10 w s1))"
              by (metis (no_types) assms mem_read2_low_equal traps_low_equal)
            then have f2: "\<And>s. \<not> low_equal (snd (memory_read 10 (address + 4) s1)) s \<or> low_equal t1 (s\<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s2)))\<rparr>)"
              using f1 by presburger
            have "snd (memory_read 10 (address + 4) s2) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s2)))\<rparr> = t2"
              using \<open>t1 = snd (memory_read 10 (address + 4) s1) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s1)))\<rparr> \<and> t2 = snd (memory_read 10 (address + 4) s2) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (address + 4) s2)))\<rparr>\<close> by presburger
            then show ?thesis
              using f2 assms mem_read2_low_equal by blast
          qed                
        next
          case False
          then have f9: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)))
                         (snd (memory_read 10 (address + 4) s1)) \<noteq> None"
            by auto
          then have f10: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2)))
                         (snd (memory_read 10 (address + 4) s2)) \<noteq> None"
            using mem_read2_low_equal user_reg_val_low_equal memory_write_low_equal memory_write_low_equal_none a1
          proof -
            have f1: "user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)) = user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2))"
              by (meson a1 mem_read2_low_equal user_reg_val_low_equal)
            have "low_equal (snd (memory_read 10 (address + 4) s2)) (snd (memory_read 10 (address + 4) s1))"
              by (meson a1 low_equal_com mem_read2_low_equal)
            then show ?thesis
              using f1 False memory_write_low_equal_none by fastforce
          qed              
          then show ?thesis 
          proof (cases "memory_write 10 (address + 4) 15
                           (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1)))
                           (case memory_write 10 address 15
                                  (user_reg_val curr_win (rd AND 30)
                                    (snd (memory_read 10 (address + 4) s1)))
                                  (snd (memory_read 10 (address + 4) s1)) of
                            Some v \<Rightarrow> v) =
                          None")            
            case True
            then have f11: "memory_write 10 (address + 4) 15
                           (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1)))
                           (case memory_write 10 address 15
                                  (user_reg_val curr_win (rd AND 30)
                                    (snd (memory_read 10 (address + 4) s1)))
                          ...
                          None"  
              by auto
            then have f12: "memory_write 10 (address + 4) 15
                           (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s2)))
                           (case memory_write 10 address 15
                                  (user_reg_val curr_win (rd AND 30)
                                    (snd (memory_read 10 (address + 4) s2)))
                                  (snd (memory_read 10 (address + 4) s2)) of
                            Some v \<Rightarrow> v) =
                          None"
              using f9 f10 apply auto
              using mem_read2_low_equal user_reg_val_low_equal memory_write_low_equal memory_write_low_equal_none a1
            proof -
              fix y :: "'a sparc_state" and ya :: "'a sparc_state"
              assume a1: "memory_write 10 (address + 4) 15 (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1))) y = None"
              assume a2: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2))) (snd (memory_read 10 (address + 4) s2)) = Some ya"
              assume a3: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1))) (snd (memory_read 10 (address + 4) s1)) = Some y"
              have f4: "\<And>w wa wb. user_reg_val w wa (snd (memory_read 10 wb s2)) = user_reg_val w wa (snd (memory_read 10 wb s1))"
                by (metis (no_types) assms mem_read2_low_equal user_reg_val_low_equal)
              then have "\<And>s. Some s \<noteq> Some ya \<or> \<not> low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2)) \<or> low_equal y s"
                using a3 a2 by (metis memory_write_low_equal)
              then have "low_equal y ya"
                by (metis (no_types) assms mem_read2_low_equal)
              then show "memory_write 10 (address + 4) 15 (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s2))) ya = None"
                using f4 a1 by (simp add: memory_write_low_equal_none)
            qed                                   
            have "t1 = snd (fst (raise_trap data_access_exception
                            (case memory_write 10 address 15
                                   (user_reg_val curr_win (rd AND 30)
                                     (snd (memory_read 10 (address + 4) s1)))
                                   (snd (memory_read 10 (address + 4) s1)) of
                             Some v \<Rightarrow> v))) \<and>
                  t2 = snd (fst (raise_trap data_access_exception
                            (case memory_write 10 address 15
                                   (user_reg_val curr_win (rd AND 30)
                                     (snd (memory_read 10 (address + 4) s2)))
                                   (snd (memory_read 10 (address + 4) s2)) of
            ...
            proof - 
              show ?thesis using a1
                apply simp
                   apply (subst(asm) comp_swap_sub2_def)
                   apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
                using f3 apply (simp add: case_prod_unfold)
                   apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)                  
                   using f5 apply (simp add: simpler_modify_def) 
                   using f9 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)                 
                      apply (simp add: case_prod_unfold return_def)
                   using f11 apply (simp add: simpler_modify_def)                     
                   apply (subst(asm) comp_swap_sub2_def)                     
                   apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
                   using f4 apply (simp add: case_prod_unfold)                     
                   apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)                         
                   using f6 apply (simp add: simpler_modify_def)                      
                   using f10 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def) 
                   apply (simp add: case_prod_unfold return_def)                     
                   using f12 apply (simp add: simpler_modify_def)                     
                   using a1 by auto                        
            qed                  
            then show ?thesis using a1 
            apply (simp add: comp_swap_sub2_def)                
            ...
            apply (simp add: case_prod_unfold)
            using f5 f6 apply (simp add: simpler_modify_def)
            using f9 f10 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
            apply (simp add: case_prod_unfold return_def)            
            using f11 f12 apply (simp add: simpler_modify_def)
            apply (simp add: raise_trap_def add_trap_set_def)
            apply (simp add: simpler_modify_def)     
            using f0 apply clarsimp  
            using  mod_trap_low_equal traps_low_equal a1 by metis          
          next
            case False
            then have f14: "memory_write 10 (address + 4) 15
              (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1)))
              (case memory_write 10 address 15
              (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)))
              (snd (memory_read 10 (address + 4) s1)) of Some v \<Rightarrow> v) \<noteq> None"  
              by auto
            then have f15: "memory_write 10 (address + 4) 15
              (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s2)))
              (case memory_write 10 address 15
              (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2)))
              (snd (memory_read 10 (address + 4) s2)) of Some v \<Rightarrow> v) \<noteq> None"
            proof -              
              obtain v1 where f1: "memory_write 10 address 15
              (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s1)))
              (snd (memory_read 10 (address + 4) s1)) = Some v1"
                using f9 by auto
              obtain v2 where f2: "memory_write 10 address 15
              (user_reg_val curr_win (rd AND 30) (snd (memory_read 10 (address + 4) s2)))
              (snd (memory_read 10 (address + 4) s2)) = Some v2"
                using f10 by auto
              from f1 f2 f0 a1 have f3: "low_equal v1 v2" by auto    
              from f14 f1 have "memory_write 10 (address + 4) 15
              (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1)))
              v1 \<noteq> None"  
                by auto
              then have "memory_write 10 (address + 4) 15
              (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s2)))
              v2 \<noteq> None"    
                using f3 a1 mem_read2_low_equal user_reg_val_low_equal memory_write_low_equal 
                memory_write_low_equal_none a1 low_equal_com memory_write_low_equal2
              proof -
                have "\<And>w wa wb. user_reg_val w wa (snd (memory_read 10 wb s1)) = user_reg_val w wa (snd (memory_read 10 wb s2))"
                  by (meson assms mem_read2_low_equal user_reg_val_low_equal)
                then show ?thesis
                  by (metis (no_types) \<open>memory_write 10 (address + 4) 15 (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1))) v1 \<noteq> None\<close> f3 low_equal_com memory_write_low_equal_none)
              qed  
            ...
            proof (cases "rd = 0")
              case True
              then have rd0: "rd = 0" by auto 
              have t1t2: "t1 = snd (fst (write_reg (user_reg_val curr_win (rs2 OR 1) 
                (snd (memory_read 10 (address + 4) s1))) curr_win 1 (case memory_write 10 (address + 4) 15
                (user_reg_val curr_win 1 (snd (memory_read 10 (address + 4) s1)))
                (case memory_write 10 address 15 (user_reg_val curr_win 0
                (snd (memory_read 10 (address + 4) s1))) (snd (memory_read 10 (address + 4) s1)) of
                Some v \<Rightarrow> v) of Some v \<Rightarrow> v))) \<and> 
                t2 = snd (fst (write_reg (user_reg_val curr_win (rs2 OR 1) 
                (snd (memory_read 10 (address + 4) s2))) curr_win 1 (case memory_write 10 (address + 4) 15
                (user_reg_val curr_win 1 (snd (memory_read 10 (address + 4) s2)))
                (case memory_write 10 address 15 (user_reg_val curr_win 0
                (snd (memory_read 10 (address + 4) s2))) (snd (memory_read 10 (address + 4) s2)) of
                Some v \<Rightarrow> v) of Some v \<Rightarrow> v)))"
                using a1 apply simp
                apply (subst comp_swap_sub2_def) 
                apply (subst comp_swap_sub2_def)  
                apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)                  
                using rd0 apply clarsimp  
                apply (simp add: case_prod_unfold)                   
                using f3 f4 apply clarsimp
                apply (simp add: simpler_modify_def)                                    
                apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
                using f5 f6 apply clarsimp 
                apply (simp add: case_prod_unfold return_def) 
                using f9 f10 apply clarsimp
                apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def) 
                apply (simp add: case_prod_unfold return_def)  
                apply (simp add: simpler_modify_def)   
                using f14 f15 apply clarsimp
                apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)   
                by (simp add: case_prod_unfold return_def)  
              show ?thesis using t1t2                  
              proof -
                obtain v1 where f1: "memory_write 10 address 15
                (user_reg_val curr_win 0 (snd (memory_read 10 (address + 4) s1)))
                (snd (memory_read 10 (address + 4) s1)) = Some v1"
                  using f9 rd0 by auto
                obtain v2 where f2: "memory_write 10 address 15
                (user_reg_val curr_win 0 (snd (memory_read 10 (address + 4) s2)))
                (snd (memory_read 10 (address + 4) s2)) = Some v2"
                  using f10 rd0 by auto
                from f1 f2 f0 a1 rd0 have v1v2: "low_equal v1 v2" by auto  
                obtain v3 where f3: "memory_write 10 (address + 4) 15
                (user_reg_val curr_win 1 (snd (memory_read 10 (address + 4) s1))) v1 = Some v3"    
                  using f14 f1 rd0 by auto
                obtain v4 where f4: "memory_write 10 (address + 4) 15
                (user_reg_val curr_win 1 (snd (memory_read 10 (address + 4) s2))) v2 = Some v4"    
                  using f15 f2 rd0 by auto
                have v2v3: "low_equal v3 v4" using f3 f4 v1v2
                  using a1 memory_write_low_equal user_reg_val_low_equal mem_read2_low_equal
                proof -
                  have f1: "\<forall>s sa sb w wa wb sc. (\<not> low_equal (s::'a sparc_state) sa \<or> Some sb \<noteq> memory_write 10 w wa wb s \<or> Some sc \<noteq> memory_write 10 w wa wb sa) \<or> low_equal sb sc"
                    by (meson memory_write_low_equal)
                  have "low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2))"
                    using a1 mem_read2_low_equal by blast
                  then have "Some v3 = memory_write 10 (address + 4) 15 (user_reg_val curr_win 1 (snd (memory_read 10 (address + 4) s2))) v1"
                    using f3 user_reg_val_low_equal by fastforce
                  then show ?thesis
                    using f1 f4 v1v2 by presburger
                qed
                then have "low_equal (snd (fst (write_reg
                    (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s1)))
                    curr_win 1 v3))) 
                      (snd (fst (write_reg
                    (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s2)))
                    curr_win 1 v4)))"    
                  using a1 write_reg_low_equal user_reg_val_low_equal mem_read2_low_equal
                proof -
                  have f1: "\<forall>s sa sb w wa wb sc. (\<not> low_equal s sa \<or> sb \<noteq> snd (fst (write_reg w (wa::'a word) wb s)) \<or> sc \<noteq> snd (fst (write_reg w wa wb sa))) \<or> low_equal sb sc"
                    by (meson write_reg_low_equal)
                  have f2: "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
                    by (meson user_reg_val_low_equal)
                  have "low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2))"
                    by (meson a1 mem_read2_low_equal)
                  then show ?thesis
                    using f2 f1 v2v3 by presburger
                qed                                      
                then show ?thesis using t1t2 f3 f4 f1 f2 rd0 by auto                    
              qed  
            next
              case False
              then have rdn0: "rd \<noteq> 0" by auto  
              have t1t2: "t1 = snd (fst (write_reg
                    (user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1)))
                    curr_win (rd AND 30)
                    (snd (fst (write_reg
                                (user_reg_val curr_win (rs2 OR 1)
                                  (snd (memory_read 10 (address + 4) s1)))
                                curr_win (rd OR 1)
                                (case memory_write 10 (address + 4) 15
                                       (user_reg_val curr_win (rd OR 1)
                                         (snd (memory_read 10 (address + 4) s1)))
                                       (case memory_write 10 address 15
                                              (user_reg_val curr_win (rd AND 30)
                                                (snd (memory_read 10 (address + 4) s1)))
                                              (snd (memory_read 10 (address + 4) s1)) of
                                        Some v \<Rightarrow> v) of
                                 Some v \<Rightarrow> v)))))) \<and>
                  t2 = snd (fst (write_reg
                    (user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1)))
                    curr_win (rd AND 30)
                    (snd (fst (write_reg
                                (user_reg_val curr_win (rs2 OR 1)
              ...
              then show ?thesis
              proof -
                obtain v1 where f1: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30)
                (snd (memory_read 10 (address + 4) s1))) (snd (memory_read 10 (address + 4) s1)) = Some v1"
                  using f9 by auto
                obtain v2 where f2: "memory_write 10 address 15 (user_reg_val curr_win (rd AND 30)
                (snd (memory_read 10 (address + 4) s2))) (snd (memory_read 10 (address + 4) s2)) = Some v2"
                  using f10 by auto
                from f1 f2 f0 a1 have v1v2: "low_equal v1 v2" by auto
                obtain v3 where f3: "memory_write 10 (address + 4) 15 (user_reg_val curr_win (rd OR 1)
                (snd (memory_read 10 (address + 4) s1))) v1 = Some v3"
                  using f1 f14 by auto
                obtain v4 where f4: "memory_write 10 (address + 4) 15 (user_reg_val curr_win (rd OR 1)
                (snd (memory_read 10 (address + 4) s2))) v2 = Some v4"
                  using f2 f15 by auto
                have v3v4: "low_equal v3 v4" using f3 f4 v1v2 a1 memory_write_low_equal 
                  user_reg_val_low_equal mem_read2_low_equal
                proof -
                  have f1: "\<forall>s sa sb w wa wb sc. (\<not> low_equal (s::'a sparc_state) sa \<or> Some sb \<noteq> memory_write 10 w wa wb s \<or> Some sc \<noteq> memory_write 10 w wa wb sa) \<or> low_equal sb sc"
                    by (meson memory_write_low_equal)
                  have "low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2))"
                    by (metis a1 mem_read2_low_equal)
                  then have "Some v3 = memory_write 10 (address + 4) 15 (user_reg_val curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s2))) v1"
                    using f3 user_reg_val_low_equal by fastforce
                  then show ?thesis
                    using f1 f4 v1v2 by presburger
                qed
                then show ?thesis using f1 f2 f3 f4 v1v2 t1t2
                  apply auto
                  using a1 user_reg_val_low_equal mem_read2_low_equal write_reg_low_equal
                proof -
                  assume a1: "low_equal v3 v4"
                  assume a2: "t2 = snd (fst (write_reg (user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1))) curr_win (rd AND 30) (snd (fst (write_reg (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s2))) curr_win (rd OR 1) v4)))))"
                  then have "\<exists>s. low_equal (snd (memory_read 10 (address + 4) s1)) s \<and> low_equal (snd (fst (write_reg (user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1))) curr_win (rd AND 30) (snd (fst (write_reg (user_reg_val curr_win (rs2 OR 1) s) curr_win (rd OR 1) v3)))))) t2"
                    using a1 by (meson assms mem_read2_low_equal write_reg_low_equal)
                  then show "low_equal (snd (fst (write_reg (user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1))) curr_win (rd AND 30) (snd (fst (write_reg (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s1))) curr_win (rd OR 1) v3)))))) (snd (fst (write_reg (user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1))) curr_win (rd AND 30) (snd (fst (write_reg (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s2))) curr_win (rd OR 1) v4))))))"
    ...
    next
      case False
      then have f16: "\<not> (word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1)) \<and>
      fst (memory_read 10 (address + 4) s1) =
      Some (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s1))))"
        by auto
      then have f17: "\<not> (word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s2)) \<and>
      fst (memory_read 10 (address + 4) s2) =
      Some (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s2))))"
        using mem_read_low_equal mem_read2_low_equal user_reg_val_low_equal memory_write_low_equal
      proof -
        have f1: "fst (memory_read 10 (address + 4) s1) = fst (memory_read 10 (address + 4) s2)"
          using a1 mem_read_low_equal by blast
        have f2: "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
          by (meson user_reg_val_low_equal)
        have "low_equal (snd (memory_read 10 (address + 4) s1)) (snd (memory_read 10 (address + 4) s2))"
      ...
      qed                  
      from f16 have f18: "\<forall>y. fst (memory_read 10 (address + 4) s1) = Some y \<longrightarrow>
        \<not> (word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s1)) \<and>      
           y =  (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s1))))"
        by blast  
      from f17 have f19: "\<forall>y. fst (memory_read 10 (address + 4) s2) = Some y \<longrightarrow>
        \<not> (word0 = user_reg_val curr_win (rs2 AND 30) (snd (memory_read 10 (address + 4) s2)) \<and>      
           y =  (user_reg_val curr_win (rs2 OR 1) (snd (memory_read 10 (address + 4) s2))))"
        by blast             
      have f20: "\<forall>y. (fst (memory_read 10 (address + 4) s1) = Some y \<and> rd = 0) \<longrightarrow>
        t1 = snd (fst (write_reg y curr_win 1 (snd (memory_read 10 (address + 4) s1))))"
        using a1 apply (subst(asm) comp_swap_sub2_def)
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)          
        using f3 f4 apply (simp add: case_prod_unfold)          
        apply clarsimp                
        apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)  
        apply (simp add: case_prod_unfold)
        apply (simp add: simpler_modify_def)
        using f18 f19 by clarsimp   
      have f21: "\<forall>y. (fst (memory_read 10 (address + 4) s2) = Some y \<and> rd = 0) \<longrightarrow>
        t2 = snd (fst (write_reg y curr_win 1 (snd (memory_read 10 (address + 4) s2))))"          
        proof -
          from a1 have "low_equal s1 s2 \<and>
            t2 = snd (fst (comp_swap_sub2 address 10 rs2 rd curr_win word0 s2))"
            by auto
          then show ?thesis 
            apply (subst(asm) comp_swap_sub2_def)
        ...
        proof (cases "rd = 0")
          ...
          then show ?thesis using a1   
            ...
            using f18 f19 apply clarsimp                           
            using a1 apply clarsimp
            using mem_read2_low_equal write_reg_low_equal user_reg_val_low_equal memory_write_low_equal
          proof -
            fix y :: word32 and ya :: word32
            assume a1: "low_equal s1 s2"
            assume a2: "fst (memory_read 10 (address + 4) s1) = Some y"
            assume a3: "fst (memory_read 10 (address + 4) s2) = Some ya"
            have f4: "\<forall>s sa. \<not> low_equal (s::'a sparc_state) sa \<or> low_equal sa s"
              by (meson low_equal_com)
            then have f5: "low_equal s2 s1"
              using a1 by metis
            have "(10::word8) \<in> {8, 10}"
              by (metis insertCI)
            then have "ya = y"
              using a3 a2 a1 by (metis (no_types) mem_read_low_equal option.sel)
            then show "low_equal (snd (fst (write_reg y curr_win 1 (snd (memory_read 10 (address + 4) s1))))) (snd (fst (write_reg ya curr_win 1 (snd (memory_read 10 (address + 4) s2)))))"
              using f5 f4 by (meson mem_read2_low_equal write_reg_low_equal)
          qed              
        next
          case False            
          then show ?thesis using a1
          ...
          proof -
            ...
            assume a2: "fst (memory_read 10 (address + 4) s1) = Some y"
            assume a3: "fst (memory_read 10 (address + 4) s2) = Some ya"
            have "Some y = fst (memory_read 10 (address + 4) s2)"
              using a2 a1 by (simp add: mem_read_low_equal)
            then have "Some y = Some ya"
              using a3 by presburger
            then have "ya = the (Some y)"
              by (metis option.sel)
            then have "ya = y"
              by (metis option.sel)
            then show "low_equal (snd (fst (write_reg word0 curr_win (rd AND 30) (snd (fst (write_reg y curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s1)))))))) (snd (fst (write_reg word0 curr_win (rd AND 30) (snd (fst (write_reg ya curr_win (rd OR 1) (snd (memory_read 10 (address + 4) s2))))))))"
              using a1 by (meson mem_read2_low_equal write_reg_low_equal)
          qed              
        qed
    qed             
  qed
  
lemma comp_swap_sub1_low_equal: 
assumes a1: "low_equal s1 s2 \<and>
(fst instr = load_store_type CASA \<or>
fst instr = load_store_type CASXA) \<and>
t1 = snd (fst (comp_swap_sub1 instr rs1 rs2 rd 0 10 s1)) \<and> 
t2 = snd (fst (comp_swap_sub1 instr rs1 rs2 rd 0 10 s2))" 
shows "low_equal t1 t2"  
proof (cases "fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))))::word2) \<noteq> 0")
  case True
  then have f1: "fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))))::word2) \<noteq> 0"
    by auto 
  then have f2: "fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                (snd (fst (get_curr_win () s2)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                (snd (fst (get_curr_win () s2)))))::word2) \<noteq> 0"  
    using a1 user_reg_val_low_equal get_curr_win_low_equal get_curr_win2_low_equal
  proof -
    have f1: "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
      by (meson assms get_curr_win_low_equal)
    have f2: "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
      using user_reg_val_low_equal by blast
    have "low_equal (snd (fst (get_curr_win () s2))) (snd (fst (get_curr_win () s1)))"
      by (meson assms get_curr_win2_low_equal low_equal_com)
    then show ?thesis
      using f2 f1 f1
      using True by auto 
  qed      
  show ?thesis using a1
    ...
    apply (simp add: raise_trap_def add_trap_set_def)
    apply (simp add: simpler_modify_def)  
    using get_curr_win2_low_equal mod_trap_low_equal traps_low_equal
  proof -
    assume "low_equal s1 s2 \<and> (fst instr = load_store_type CASA \<or> fst instr = load_store_type CASXA) \<and> t1 = snd (fst (get_curr_win () s1)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s1))))\<rparr> \<and> t2 = snd (fst (get_curr_win () s2)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s2))))\<rparr>"
    then show "low_equal (snd (fst (get_curr_win () s1)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s1))))\<rparr>) (snd (fst (get_curr_win () s2)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s2))))\<rparr>)"
      by (metis (no_types) get_curr_win2_low_equal mod_trap_low_equal traps_low_equal)
  qed             
next
  case False
  then have f3: "\<not>(fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))))::word2) \<noteq> 0)"  
    by auto
  then have f4: "\<not>(fst instr = load_store_type CASXA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                (snd (fst (get_curr_win () s2)))))::word3) \<noteq> 0 \<or>
              fst instr = load_store_type CASA \<and>
              ((ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1
    ...
    using a1 user_reg_val_low_equal get_curr_win_low_equal get_curr_win2_low_equal
  proof -
    have f1: "fst (fst (get_curr_win () s2)) = fst (fst (get_curr_win () s1))"
      by (metis (no_types) assms get_curr_win_low_equal)
    have "\<And>w wa. user_reg_val w wa (snd (fst (get_curr_win () s2))) = user_reg_val w wa (snd (fst (get_curr_win () s1)))"
      by (metis (no_types) assms get_curr_win2_low_equal user_reg_val_low_equal)
    then show ?thesis
      using f1 f3 by presburger
  qed      
  then show ?thesis 
  proof (cases "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = None")
    case True
    then have f5: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = None"
      by auto
    then have f6: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) = None"
      using a1 get_curr_win_low_equal get_curr_win2_low_equal mem_read_low_equal user_reg_val_low_equal
    proof -
      have f1: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
        by (meson a1 get_curr_win2_low_equal)
      have f2: "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
        by (meson a1 get_curr_win_low_equal)
      have "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s2))))"
        using f1 by (meson insert_iff mem_read_low_equal)
      then show ?thesis
        using f2 f1 True user_reg_val_low_equal by fastforce
    qed        
    then show ?thesis using a1
    apply (simp add: comp_swap_sub1_def)
    apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
    using f3 f4 apply (simp add: case_prod_unfold)  
    using f5 f6 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
    apply (simp add: case_prod_unfold return_def)
    apply (simp add: raise_trap_def add_trap_set_def)
    apply (simp add: simpler_modify_def)  
    using get_curr_win2_low_equal mod_trap_low_equal traps_low_equal
  proof -
    assume a1: "low_equal s1 s2 \<and> (fst instr = load_store_type CASA \<or> fst instr = load_store_type CASXA) \<and> t1 = snd (fst (get_curr_win () s1)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s1))))\<rparr> \<and> t2 = snd (fst (get_curr_win () s2)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s2))))\<rparr>"
    assume a2: "(fst instr = load_store_type CASXA \<longrightarrow> ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) = (0::word3)) \<and> (fst instr = load_store_type CASA \<longrightarrow> ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) = (0::word2))"
    assume a3: "(fst instr = load_store_type CASXA \<longrightarrow> ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) = (0::word3)) \<and> (fst instr = load_store_type CASA \<longrightarrow> ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) = (0::word2))"
    have f4: "\<forall>s sa. snd (fst (get_curr_win () s2)) \<noteq> s \<or> low_equal sa s \<or> snd (fst (get_curr_win () s1)) \<noteq> sa"
      using a1 by (meson get_curr_win2_low_equal)
    then have "traps (snd (fst (get_curr_win () s1))) = traps (snd (fst (get_curr_win () s2)))"
      using traps_low_equal by blast
    then have "snd (fst (get_curr_win () s2)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s1))))\<rparr> = t2"
      using a1 by presburger
    then show "(fst instr = load_store_type CASXA \<and> ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) \<noteq> (0::word3) \<longrightarrow> low_equal (snd (fst (get_curr_win () s1)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s1))))\<rparr>) (snd (fst (get_curr_win () s2)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s2))))\<rparr>)) \<and> (fst instr = load_store_type CASA \<and> ucast (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) \<noteq> (0::word2) \<longrightarrow> low_equal (snd (fst (get_curr_win () s1)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s1))))\<rparr>) (snd (fst (get_curr_win () s2)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s2))))\<rparr>)) \<and> (fst instr = load_store_type CASXA \<and> ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) \<noteq> (0::word3) \<longrightarrow> low_equal (snd (fst (get_curr_win () s1)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s1))))\<rparr>) (snd (fst (get_curr_win () s2)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s2))))\<rparr>)) \<and> (fst instr = load_store_type CASA \<and> ucast (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) \<noteq> (0::word2) \<longrightarrow> low_equal (snd (fst (get_curr_win () s1)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s1))))\<rparr>) (snd (fst (get_curr_win () s2)) \<lparr>traps := insert mem_address_not_aligned (traps (snd (fst (get_curr_win () s2))))\<rparr>)) \<and> low_equal (snd (fst (get_curr_win () s1)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s1))))\<rparr>) (snd (fst (get_curr_win () s2)) \<lparr>traps := insert data_access_exception (traps (snd (fst (get_curr_win () s2))))\<rparr>)"
      using f4 a3 a2 a1 by (metis (full_types) mod_trap_low_equal)
  qed      
  next
    case False
    then have f7: "\<not>(fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = None)" 
      by auto
    then have f8: "\<not>(fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) = None)" 
      using a1 get_curr_win_low_equal get_curr_win2_low_equal mem_read_low_equal user_reg_val_low_equal
    proof -
      have f1: "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
        by (meson assms get_curr_win_low_equal)
      have f2: "\<And>w wa. user_reg_val w wa (snd (fst (get_curr_win () s1))) = user_reg_val w wa (snd (fst (get_curr_win () s2)))"
        by (meson assms get_curr_win2_low_equal user_reg_val_low_equal)
      have "\<And>w wa. w \<noteq> 10 \<or> fst (memory_read w wa (snd (fst (get_curr_win () s1)))) = fst (memory_read w wa (snd (fst (get_curr_win () s2))))"
        by (meson assms get_curr_win2_low_equal insert_iff mem_read_low_equal)
      then show ?thesis
        using f2 f1 f7 by presburger
    qed        
    then show ?thesis 
    proof (cases "fst instr = load_store_type CASA")
      case True
      then have f9: "fst instr = load_store_type CASA"
        by auto
      then show ?thesis 
      proof (cases "(case fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                    (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) of
                        Some v \<Rightarrow> v) = user_reg_val (fst (fst (get_curr_win () s1))) rs2
                        (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                        (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))")
        case True
        then have f10: "(case fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                    (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) of
                        Some v \<Rightarrow> v) = user_reg_val (fst (fst (get_curr_win () s1))) rs2
                        (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                        (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))"
          by auto
        then have f11: "(case fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                    (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) of
                        Some v \<Rightarrow> v) = user_reg_val (fst (fst (get_curr_win () s2))) rs2
                        (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                        (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
          using a1 get_curr_win_low_equal get_curr_win2_low_equal mem_read_low_equal user_reg_val_low_equal
        proof -
          have f1: "fst (fst (get_curr_win () s2)) = fst (fst (get_curr_win () s1))"
            by (metis (no_types) assms get_curr_win_low_equal)
          have f2: "\<And>w wa. user_reg_val w wa (snd (fst (get_curr_win () s2))) = user_reg_val w wa (snd (fst (get_curr_win () s1)))"
            by (metis (no_types) assms get_curr_win2_low_equal user_reg_val_low_equal)
          have f3: "\<And>w wa. w \<noteq> 10 \<or> fst (memory_read w wa (snd (fst (get_curr_win () s2)))) = fst (memory_read w wa (snd (fst (get_curr_win () s1))))"
            by (metis (no_types) assms get_curr_win2_low_equal insert_iff mem_read_low_equal)
          have "\<And>w wa wb. user_reg_val w wa (snd (memory_read 10 wb (snd (fst (get_curr_win () s2))))) = user_reg_val w wa (snd (memory_read 10 wb (snd (fst (get_curr_win () s1)))))"
            by (metis (no_types) assms get_curr_win2_low_equal mem_read2_low_equal user_reg_val_low_equal)
          then show ?thesis
            using f3 f2 f1 f10 by presburger
        qed            
        show ?thesis
        proof (cases "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                      (snd (fst (get_curr_win () s1)))) 15 (user_reg_val (fst (fst (get_curr_win () s1))) rd
                      (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                      (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))))
                      (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                      (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) = None")
          case True
          then have f12: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                      (snd (fst (get_curr_win () s1)))) 15 (user_reg_val (fst (fst (get_curr_win () s1))) rd
                      (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                      (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))))
                      (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
                      (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) = None"
            by auto
          then have f13: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                      (snd (fst (get_curr_win () s2)))) 15 (user_reg_val (fst (fst (get_curr_win () s2))) rd
                      (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                      (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))))
                      (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
                      (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))) = None"
            using a1 get_curr_win_low_equal get_curr_win2_low_equal mem_read_low_equal mem_read2_low_equal memory_write_low_equal_none user_reg_val_low_equal
          proof -
            have f1: "fst (fst (get_curr_win () s2)) = fst (fst (get_curr_win () s1))"
              by (metis (no_types) assms get_curr_win_low_equal)
            have f2: "\<And>w wa. user_reg_val w wa (snd (fst (get_curr_win () s2))) = user_reg_val w wa (snd (fst (get_curr_win () s1)))"
              by (metis (no_types) assms get_curr_win2_low_equal user_reg_val_low_equal)
            have f3: "\<And>w wa wb. user_reg_val w wa (snd (memory_read 10 wb (snd (fst (get_curr_win () s2))))) = user_reg_val w wa (snd (memory_read 10 wb (snd (fst (get_curr_win () s1)))))"
              by (metis (no_types) assms get_curr_win2_low_equal mem_read2_low_equal user_reg_val_low_equal)
            have "low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s2)))))"
              by (meson assms get_curr_win2_low_equal mem_read2_low_equal)
            then show ?thesis
              using f3 f2 f1 by (simp add: f12 memory_write_low_equal_none)
          qed           
          from a1 f9 have f14: "t1 = snd (memory_read 10
               (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1))))
               (snd (fst (get_curr_win () s1)))) \<lparr>traps := insert data_access_exception
               (traps (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1
               (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))))\<rparr>" 
            apply auto
            apply (subst comp_swap_sub1_def)  
            apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
            using f3 f4 apply (simp add: case_prod_unfold)
            using f7 f8 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
            using f9 apply (simp add: case_prod_unfold return_def)  
            using f10 f11 apply (simp add: simpler_modify_def)  
            apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
            using f12 f13 apply auto 
              apply (simp add: case_prod_unfold return_def)
            apply (simp add: raise_trap_def add_trap_set_def)
            apply (simp add: simpler_modify_def) 
             apply (simp add: case_prod_unfold return_def)
            ...
            apply (simp add: simpler_modify_def)
            by (simp add: case_prod_unfold return_def)
          from a1 f9 have f15: "t2 = snd (memory_read 10
               (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2))))
               (snd (fst (get_curr_win () s2)))) \<lparr>traps := insert data_access_exception
               (traps (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1
               (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))))\<rparr>" 
            apply auto
            apply (subst comp_swap_sub1_def)  
            apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
            using f3 f4 apply (simp add: case_prod_unfold)
          ...
          show ?thesis 
            using f14 f15 apply auto
            using a1 get_curr_win2_low_equal mod_trap_low_equal traps_low_equal 
                  get_curr_win_low_equal mem_read_low_equal mem_read2_low_equal 
                  memory_write_low_equal_none user_reg_val_low_equal
          proof -
            assume a1: "t1 = snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))))\<rparr>"
            have f2: "\<And>u. get_curr_win u = gets (\<lambda>s. ucast (get_CWP (cpu_reg_val PSR s))::'a word)"
              using get_curr_win_def by auto
            have f3: "fst (fst (get_curr_win () s2)) = fst (fst (get_curr_win () s1))"
              by (metis (no_types) assms get_curr_win_low_equal)
            have f4: "\<And>w wa. user_reg_val w wa s2 = user_reg_val w wa s1"
              by (metis assms user_reg_val_low_equal)
            have f5: "\<And>f s. fst (gets f (s::'a sparc_state)) = (f s::'a word, s)"
              by (metis fst_conv simpler_gets_def)
            have f6: "\<And>u. fst (fst (get_curr_win u s2)) = fst (fst (get_curr_win () s1))"
              using f3 by auto
            then have f7: "fst (fst (get_curr_win () s1)) = ucast (get_CWP (cpu_reg_val PSR s2))"
              using f5 f2 by simp
            have f8: "(ucast (get_CWP (cpu_reg_val PSR s2))::'a word) = ucast (get_CWP (cpu_reg_val PSR s1))"
              using f6 f5 f2 by auto
            have f9: "((\<lambda>T. insert data_access_exception (traps (snd (memory_read 10 (user_reg_val (ucast (get_CWP (cpu_reg_val PSR s1))) rs1 s1) s2))))::Trap set \<Rightarrow> Trap set) = (\<lambda>T. insert data_access_exception (traps (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))))"
              using f5 f4 f3 f2 by auto
            have "\<And>w. traps (snd (memory_read 10 w s2)) = traps (snd (memory_read 10 w s1))"
              by (meson assms low_equal_com mem_read2_low_equal traps_low_equal)
            then show "low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))))\<rparr>) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) \<lparr>traps := insert data_access_exception (traps (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))))\<rparr>)"
              using f9 f8 f7 f6 f5 f4 f3 f2 a1 by (metis (no_types) assms mem_read2_low_equal mod_trap_low_equal snd_conv)
          qed                                                     
        next
          case False
          then have f16: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) 15
            (user_reg_val (fst (fst (get_curr_win () s1))) rd (snd (memory_read 10
            (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1))))
            (snd (fst (get_curr_win () s1)))))) (snd (memory_read 10
            (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1))))
            (snd (fst (get_curr_win () s1))))) \<noteq> None" 
            by auto
          then have f17: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) 15
            (user_reg_val (fst (fst (get_curr_win () s2))) rd (snd (memory_read 10
            (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2))))
            (snd (fst (get_curr_win () s2)))))) (snd (memory_read 10
            (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2))))
            (snd (fst (get_curr_win () s2))))) \<noteq> None"
            using a1 get_curr_win_low_equal get_curr_win2_low_equal mem_read_low_equal mem_read2_low_equal memory_write_low_equal_none user_reg_val_low_equal
          proof -
            have f1: "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
              by (meson assms get_curr_win_low_equal)
            have f2: "\<And>w wa. user_reg_val w wa (snd (fst (get_curr_win () s1))) = user_reg_val w wa (snd (fst (get_curr_win () s2)))"
              by (metis (no_types) assms get_curr_win2_low_equal user_reg_val_low_equal)
            have "low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s1)))))"
              by (meson assms get_curr_win2_low_equal low_equal_com mem_read2_low_equal)
            then show ?thesis
              using f2 f1 f16 memory_write_low_equal_none user_reg_val_low_equal by fastforce
          qed              
          then show ?thesis 
          proof (cases "rd = 0")
            case True
            then show ?thesis 
              using a1 apply (simp add: comp_swap_sub1_def)
              apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
              using f3 f4 apply (simp add: case_prod_unfold)  
              using f7 f8 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
              using f9 apply (simp add: case_prod_unfold return_def)
              using f10 f11 apply (simp add: simpler_modify_def)
              apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
              using f16 f17 apply clarsimp  
              apply (simp add: simpler_modify_def)  
              apply (simp add: case_prod_unfold return_def)  
              using get_curr_win2_low_equal mod_trap_low_equal traps_low_equal 
                  get_curr_win_low_equal mem_read_low_equal mem_read2_low_equal 
                  memory_write_low_equal_none user_reg_val_low_equal memory_write_low_equal
            proof -
              fix yb :: "'a sparc_state" and yc :: "'a sparc_state"
              assume a1: "low_equal s1 s2"
              assume a2: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) 15 (user_reg_val (fst (fst (get_curr_win () s1))) 0 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) = Some yb"
              assume a3: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) 15 (user_reg_val (fst (fst (get_curr_win () s2))) 0 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))) = Some yc"
              have f4: "\<forall>s sa sb w sc. (\<not> low_equal (s::'a sparc_state) sa \<or> sb \<noteq> snd (memory_read 10 w s) \<or> sc \<noteq> snd (memory_read 10 w sa)) \<or> low_equal sb sc"
                by (metis mem_read2_low_equal)
              have f5: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
                using a1 by (metis get_curr_win2_low_equal)
              have f6: "Some yb = memory_write 10 (if rs1 = 0 then 0 else user_reg (snd (fst (get_curr_win () s1))) (fst (fst (get_curr_win () s1))) rs1) 15 0 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))"
                using a2 by (simp add: user_reg_val_def)
              have "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
                using a1 by (meson get_curr_win_low_equal)
              then have f7: "(if rs1 = 0 then 0 else user_reg (snd (fst (get_curr_win () s1))) (fst (fst (get_curr_win () s1))) rs1) = user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))"
                using f5 by (metis (no_types) user_reg_val_def user_reg_val_low_equal)
              then have f8: "Some yc = memory_write 10 (if rs1 = 0 then 0 else user_reg (snd (fst (get_curr_win () s1))) (fst (fst (get_curr_win () s1))) rs1) 15 0 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
                using a3 by (simp add: user_reg_val_def)
              have "low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
                using f7 f5 f4 by (simp add: user_reg_val_def)
              then show "low_equal yb yc"
                using f8 f6 by (meson memory_write_low_equal)
            qed                  
          next
            case False
            then show ?thesis 
            ...
            proof -
              fix yb :: "'a sparc_state" and yc :: "'a sparc_state"
              assume a1: "low_equal s1 s2"
              assume a2: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) 15 (user_reg_val (fst (fst (get_curr_win () s1))) rd (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) = Some yb"
              assume a3: "memory_write 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) 15 (user_reg_val (fst (fst (get_curr_win () s2))) rd (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))) = Some yc"
              have f4: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
                using a1 by (metis get_curr_win2_low_equal)
              have f5: "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
                using a1 by (meson get_curr_win_low_equal)
              have f6: "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
                by (meson user_reg_val_low_equal)
              then have f7: "low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
                using f5 f4 mem_read2_low_equal by fastforce
              then have "Some yc = memory_write 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) 15 (user_reg_val (fst (fst (get_curr_win () s1))) rd (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
                using f6 f5 f4 a3 by force
              then have "low_equal yb yc"
                using f7 a2 by (metis (no_types) memory_write_low_equal)
              then show "low_equal (snd (fst (write_reg (user_reg_val (fst (fst (get_curr_win () s1))) rs2 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))) (fst (fst (get_curr_win () s1))) rd yb))) (snd (fst (write_reg (user_reg_val (fst (fst (get_curr_win () s2))) rs2 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))) (fst (fst (get_curr_win () s2))) rd yc)))"
                using f7 f6 f5 write_reg_low_equal by fastforce
            qed                  
          qed                               
      qed                             
    next
      case False
      then have f18: "(case fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) 
        rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) of Some v \<Rightarrow> v) \<noteq>
        user_reg_val (fst (fst (get_curr_win () s1))) rs2 (snd (memory_read 10
        (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1))))
        (snd (fst (get_curr_win () s1)))))"  
        by auto
      then have f19: "(case fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) 
        rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) of Some v \<Rightarrow> v) \<noteq>
        user_reg_val (fst (fst (get_curr_win () s2))) rs2 (snd (memory_read 10
        (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2))))
        (snd (fst (get_curr_win () s2)))))" 
        using a1 get_curr_win_low_equal get_curr_win2_low_equal mem_read_low_equal user_reg_val_low_equal
      proof -
        have f1: "\<forall>s sa sb w sc. (\<not> low_equal (s::'a sparc_state) sa \<or> sb \<noteq> snd (memory_read 10 w s) \<or> sc \<noteq> snd (memory_read 10 w sa)) \<or> low_equal sb sc"
          by (meson mem_read2_low_equal)
        have f2: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
          by (meson a1 get_curr_win2_low_equal)
        have f3: "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
          by (meson user_reg_val_low_equal)
        have f4: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s2))))"
          using f2 by (meson insert_iff mem_read_low_equal)
        have f5: "\<forall>s sa. \<not> low_equal s sa \<or> (fst (fst (get_curr_win () s))::'a word) = fst (fst (get_curr_win () sa))"
          by (meson get_curr_win_low_equal)
        then have "user_reg_val (fst (fst (get_curr_win () s2))) rs2 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) = user_reg_val (fst (fst (get_curr_win () s2))) rs2 (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
          using f3 f2 f1 a1 by force
        then show ?thesis
          using f5 f4 f3 f2 False a1 by force
      qed          
      then show ?thesis 
      proof (cases "rd = 0")
        case True
        then show ?thesis 
          ...
          using f7 f8 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
          using f9 apply (simp add: case_prod_unfold return_def)
          using f18 f19 apply (simp add: simpler_modify_def return_def)
          using get_curr_win2_low_equal get_curr_win_low_equal mem_read2_low_equal user_reg_val_low_equal
        proof -
          assume a1: "low_equal s1 s2 \<and> t1 = snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) \<and> t2 = snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))"
          have f2: "\<forall>s sa sb w sc. (\<not> low_equal (s::'a sparc_state) sa \<or> sb \<noteq> snd (memory_read 10 w s) \<or> sc \<noteq> snd (memory_read 10 w sa)) \<or> low_equal sb sc"
            by (meson mem_read2_low_equal)
          have f3: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
            using a1 by (meson get_curr_win2_low_equal)
          have f4: "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
            using a1 by (meson get_curr_win_low_equal)
          have "\<forall>s sa w wa. \<not> low_equal s sa \<or> user_reg_val (w::'a word) wa s = user_reg_val w wa sa"
            by (metis user_reg_val_low_equal)
          then show "(fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = None \<longrightarrow> (fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) = None \<longrightarrow> low_equal (snd (fst (raise_trap data_access_exception (snd (fst (get_curr_win () s1)))))) (snd (fst (raise_trap data_access_exception (snd (fst (get_curr_win () s2))))))) \<and> low_equal (snd (fst (raise_trap data_access_exception (snd (fst (get_curr_win () s1)))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))) \<and> (fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) = None \<longrightarrow> low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) (snd (fst (raise_trap data_access_exception (snd (fst (get_curr_win () s2))))))) \<and> low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))"
            using f4 f3 f2 f7 f8 by presburger
        qed                                  
      next
        case False
        then show ?thesis 
        ...
        proof -
          fix y :: word32 and ya :: word32
          assume a1: "low_equal s1 s2"
          assume a2: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = Some y"
          assume a3: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) = Some ya"
          have f4: "\<forall>s sa sb w sc. (\<not> low_equal (s::'a sparc_state) sa \<or> sb \<noteq> snd (memory_read 10 w s) \<or> sc \<noteq> snd (memory_read 10 w sa)) \<or> low_equal sb sc"
            by (meson mem_read2_low_equal)
          have f5: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
            using a1 by (metis get_curr_win2_low_equal)
          have f6: "\<forall>s sa sb w wa wb sc. (\<not> low_equal s sa \<or> sb \<noteq> snd (fst (write_reg w (wa::'a word) wb s)) \<or> sc \<noteq> snd (fst (write_reg w wa wb sa))) \<or> low_equal sb sc"
            by (meson write_reg_low_equal)
          have f7: "user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1))) = user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))"
            using f5 a1 by (simp add: get_curr_win_low_equal user_reg_val_low_equal)
          then have f8: "ya = y"
            using f5 a3 a2 by (simp add: mem_read_low_equal)
          have "fst (fst (get_curr_win () s1)) = fst (fst (get_curr_win () s2))"
            using a1 by (metis get_curr_win_low_equal)
          then show "low_equal (snd (fst (write_reg y (fst (fst (get_curr_win () s1))) rd (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))))) (snd (fst (write_reg ya (fst (fst (get_curr_win () s2))) rd (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))))))"
  ...
  next
    case False
    then have f20: "fst instr \<noteq> load_store_type CASA"
      by auto
    then show ?thesis using a1
    apply (simp add: comp_swap_sub1_def)
    apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
    using f3 f4 apply (simp add: case_prod_unfold)  
    using f7 f8 apply (simp add: simpler_gets_def bind_def h1_def h2_def Let_def)
    using f20 apply (simp add: case_prod_unfold return_def)
    apply (simp add: raise_trap_def add_trap_set_def)
    apply (simp add: simpler_modify_def)
    apply auto
    using get_curr_win2_low_equal get_curr_win_low_equal mem_read_low_equal 
      mem_read2_low_equal user_reg_val_low_equal comp_swap_sub2_low_equal
  proof -
    fix y :: word32 and ya :: word32
    assume a1: "low_equal s1 s2"
    assume a2: "fst instr = load_store_type CASXA"
    assume a3: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))) = Some ya"
    assume a4: "fst (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))) = Some y"
    assume "t2 = snd (fst (comp_swap_sub2 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) 10 rs2 rd (fst (fst (get_curr_win () s2))) ya (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2)))))))"
    assume "t1 = snd (fst (comp_swap_sub2 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) 10 rs2 rd (fst (fst (get_curr_win () s1))) y (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))))"
    have f5: "low_equal (snd (fst (get_curr_win () s1))) (snd (fst (get_curr_win () s2)))"
      using a1 by (metis get_curr_win2_low_equal)
    have f6: "\<And>s sa w wa wb wc wd. \<not> low_equal s sa \<or> low_equal (snd (fst (comp_swap_sub2 w 10 wa wb (wc::'a word) wd s))) (snd (fst (comp_swap_sub2 w 10 wa wb wc wd sa)))"
      using a2 by (meson comp_swap_sub2_low_equal)
    have "low_equal (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1))))) (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s2)))))"
      using f5 by (meson mem_read2_low_equal)
    then show "low_equal (snd (fst (comp_swap_sub2 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) 10 rs2 rd (fst (fst (get_curr_win () s1))) y (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s1))) rs1 (snd (fst (get_curr_win () s1)))) (snd (fst (get_curr_win () s1)))))))) (snd (fst (comp_swap_sub2 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) 10 rs2 rd (fst (fst (get_curr_win () s2))) ya (snd (memory_read 10 (user_reg_val (fst (fst (get_curr_win () s2))) rs1 (snd (fst (get_curr_win () s2)))) (snd (fst (get_curr_win () s2))))))))"
      using f6 f5 a4 a3 a1 by (simp add: get_curr_win_low_equal mem_read_low_equal user_reg_val_low_equal)
  qed               
  qed    
qed    
qed  
  
lemma comp_swap_instr_low_equal: 
assumes a1: "low_equal s1 s2 \<and>
(fst instr = load_store_type CASA \<or>
fst instr = load_store_type CASXA) \<and>
((ucast (get_S (cpu_reg_val PSR s1)))::word1) = 0 \<and>
((ucast (get_S (cpu_reg_val PSR s2)))::word1) = 0 \<and>
t1 = snd (fst (comp_swap_instr instr s1)) \<and> t2 = snd (fst (comp_swap_instr instr s2))"
shows "low_equal t1 t2"  
proof -
  have "get_S (cpu_reg_val PSR s1) = 0 \<and> get_S (cpu_reg_val PSR s2) = 0"
    using a1 by (simp add: ucast_id) 
  ...
  apply (simp add: simpler_modify_def)
  apply (simp add: traps_low_equal)
  by (auto intro: mod_trap_low_equal comp_swap_sub1_low_equal)
qed
                              ...
                              next
                                ...
                                next
                                  ...
                                  next
                                    ...
                                    next
                                      case False
                                      then have f18: "fst instr \<notin> {bicc_type BE, bicc_type BNE, bicc_type BGU, bicc_type BLE, bicc_type BL, bicc_type BGE,
                                        bicc_type BNEG, bicc_type BG, bicc_type BCS, bicc_type BLEU, bicc_type BCC, bicc_type BA,
                                        bicc_type BN}"  
                                        by auto
                                      then show ?thesis 
                                      proof (cases "fst instr \<in> {load_store_type CASA, load_store_type CASXA}")
                                        ...
                                        f9 f10 f11 f12 f13 f14 f15 f16 f17 f18
                                          apply dispath_instr_privilege_proof
                                          using comp_swap_instr_low_equal by blast  
                                      next
                                        case False
                                        then show ?thesis 
                                          using a1 f_no_traps f1 f2 f3 f4 f5 f6 f7 f8 
                                            f9 f10 f11 f12 f13 f14 f15 f16 f17 f18
                                          apply dispath_instr_privilege_proof
                                          by (simp add: fail_def)  
                                      qed                                                                                  
  ...
  proof (cases "exe_mode_val s1")
    ...
    proof (cases "\<exists>e. fetch_instruction (delayed_pool_write s1) = Inl e")
      ...
      proof (cases "exe_mode_val s1")
        ...
        apply (simp add: cpu_reg_val_def)
        apply clarsimp
        using mem_equal_mod_trap by metis   
      next
        case False
    ...
    next
      ...
      then have "(\<exists>v. fetch_instruction (delayed_pool_write s1) = Inr v \<and>
        ...
        using dispatch_fail f0 a1 f_exe by blast
