...
where \<comment>\<open> CALL, with a single operand disp30+"00" \<close>
...
where "parse_instr_f3 w \<equiv>
  ...
  \<comment>\<open> If an instruction accesses alternative space but flagi = 1, 
    ...
    if op3 = uint(0b001001::word6) then \<comment>\<open> LDSB \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LDSB,[(Flag (word_of_int flagi)),
                        ...
                        (W5 (word_of_int rs2)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b011001::word6) then \<comment>\<open> LDSBA \<close> 
       Inr (load_store_type LDSBA,[(Flag (word_of_int flagi)),
                         (W5 (word_of_int rs1)),
                         (W5 (word_of_int rs2)),
                         (Asi (word_of_int asi)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b001010::word6) then \<comment>\<open> LDSH \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LDSH,[(Flag (word_of_int flagi)),
                        ...
                        (W5 (word_of_int rs2)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b011010::word6) then \<comment>\<open> LDSHA \<close> 
       Inr (load_store_type LDSHA,[(Flag (word_of_int flagi)),
                         (W5 (word_of_int rs1)),
                         (W5 (word_of_int rs2)),
                         (Asi (word_of_int asi)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b000001::word6) then \<comment>\<open> LDUB \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LDUB,[(Flag (word_of_int flagi)),
                        ...
                        (W5 (word_of_int rs2)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b010001::word6) then \<comment>\<open> LDUBA \<close> 
       Inr (load_store_type LDUBA,[(Flag (word_of_int flagi)),
                         (W5 (word_of_int rs1)),
                         (W5 (word_of_int rs2)),
                         (Asi (word_of_int asi)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b000010::word6) then \<comment>\<open> LDUH \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LDUH,[(Flag (word_of_int flagi)),
                        ...
                        (W5 (word_of_int rs2)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b010010::word6) then \<comment>\<open> LDUHA \<close> 
       Inr (load_store_type LDUHA,[(Flag (word_of_int flagi)),
                         (W5 (word_of_int rs1)),
                         (W5 (word_of_int rs2)),
                         (Asi (word_of_int asi)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b000000::word6) then \<comment>\<open> LD \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LD,[(Flag (word_of_int flagi)),
                      ...
                      (W5 (word_of_int rs2)),
                      (W5 (word_of_int rd))])
    else if op3 = uint(0b010000::word6) then \<comment>\<open> LDA \<close>
       Inr (load_store_type LDA,[(Flag (word_of_int flagi)),
                       (W5 (word_of_int rs1)),
                       (W5 (word_of_int rs2)),
                       (Asi (word_of_int asi)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b000011::word6) then \<comment>\<open> LDD \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LDD,[(Flag (word_of_int flagi)),
                       ...
                       (W5 (word_of_int rs2)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b010011::word6) then \<comment>\<open> LDDA \<close> 
       Inr (load_store_type LDDA,[(Flag (word_of_int flagi)),
                         (W5 (word_of_int rs1)),
                         (W5 (word_of_int rs2)),
                         (Asi (word_of_int asi)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b001101::word6) then \<comment>\<open> LDSTUB \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type LDSTUB,[(Flag (word_of_int flagi)),
                      ...
                      (W5 (word_of_int rs2)),
                      (W5 (word_of_int rd))])
    else if op3 = uint(0b011101::word6) then \<comment>\<open> LDSTUBA \<close>
       Inr (load_store_type LDSTUBA,[(Flag (word_of_int flagi)),
                       (W5 (word_of_int rs1)),
                       (W5 (word_of_int rs2)),
                       (Asi (word_of_int asi)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b000101::word6) then \<comment>\<open> STB \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type STB,[(Flag (word_of_int flagi)),
                       ...
                       (W5 (word_of_int rs2)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b010101::word6) then \<comment>\<open> STBA \<close>
       Inr (load_store_type STBA,[(Flag (word_of_int flagi)),
                       (W5 (word_of_int rs1)),
                       (W5 (word_of_int rs2)),
                       (Asi (word_of_int asi)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b000110::word6) then \<comment>\<open> STH \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type STH,[(Flag (word_of_int flagi)),
                       ...
                       (W5 (word_of_int rs2)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b010110::word6) then \<comment>\<open> STHA \<close>
      ...
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type ST,[(Flag (word_of_int flagi)),
                      ...
                      (W5 (word_of_int rs2)),
                      (W5 (word_of_int rd))])
    else if op3 = uint(0b010100::word6) then \<comment>\<open> STA \<close>
       Inr (load_store_type STA,[(Flag (word_of_int flagi)),
                       (W5 (word_of_int rs1)),
                       (W5 (word_of_int rs2)),
                       (Asi (word_of_int asi)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b000111::word6) then \<comment>\<open> STD \<close>
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type STD,[(Flag (word_of_int flagi)),
                       ...
                       (W5 (word_of_int rs2)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b010111::word6) then \<comment>\<open> STDA \<close>
      ...
      if flagi = 1 then \<comment>\<open> Operant list is [i,rs1,simm13,rd] \<close>
         ...
         Inr (load_store_type SWAP,[(Flag (word_of_int flagi)),
                       ...
                       (W5 (word_of_int rs2)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b011111::word6) then \<comment>\<open> SWAPA \<close>
       ...
       Inr (load_store_type CASA,[(Flag (word_of_int flagi)),
                       (W5 (word_of_int rs1)),
                       (W5 (word_of_int rs2)),
                       (Asi (word_of_int asi)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b111110::word6) then \<comment>\<open> CASXA \<close>
       Inr (load_store_type CASXA,[(Flag (word_of_int flagi)),
                       (W5 (word_of_int rs1)),
                       (W5 (word_of_int rs2)),
                       (Asi (word_of_int asi)),
                       (W5 (word_of_int rd))])
    ...
    if op3 = uint(0b111000::word6) then \<comment>\<open> JMPL \<close>
      ...
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
         ...
         Inr (ctrl_type RETT,[(Flag (word_of_int flagi)),
                        (W5 (word_of_int rs1)),
                        (Simm13 (word_of_int simm13))])
    \<comment>\<open> The following are Read and Write instructions, 
       only return [rs1,rd] as operand. \<close>
    else if op3 = uint(0b101000::word6) \<and> rs1 \<noteq> 0 then \<comment>\<open> RDASR \<close>
      if rs1 = uint(0b01111::word6) \<and> rd = 0 then \<comment>\<open> STBAR is a special case of RDASR \<close>
        Inr (load_store_type STBAR,[])
      else Inr (sreg_type RDASR,[(W5 (word_of_int rs1)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b101000::word6) \<and> rs1 = 0 then \<comment>\<open> RDY \<close>
       Inr (sreg_type RDY,[(W5 (word_of_int rs1)),
                     (W5 (word_of_int rd))])
    else if op3 = uint(0b101001::word6) then \<comment>\<open> RDPSR \<close>
       Inr (sreg_type RDPSR,[(W5 (word_of_int rs1)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b101010::word6) then \<comment>\<open> RDWIM \<close>
       Inr (sreg_type RDWIM,[(W5 (word_of_int rs1)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b101011::word6) then \<comment>\<open> RDTBR \<close>
       Inr (sreg_type RDTBR,[(W5 (word_of_int rs1)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b110000::word6) \<and> rd \<noteq> 0 then \<comment>\<open> WRASR \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (sreg_type WRASR,[(Flag (word_of_int flagi)),
                         ...
                         (Simm13 (word_of_int simm13)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b110000::word6) \<and> rd = 0 then \<comment>\<open> WRY \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (sreg_type WRY,[(Flag (word_of_int flagi)),
                       ...
                       (Simm13 (word_of_int simm13)),
                       (W5 (word_of_int rd))])
    else if op3 = uint(0b110001::word6) then \<comment>\<open> WRPSR \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (sreg_type WRPSR,[(Flag (word_of_int flagi)),
                         ...
                         (Simm13 (word_of_int simm13)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b110010::word6) then \<comment>\<open> WRWIM \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (sreg_type WRWIM,[(Flag (word_of_int flagi)),
                         ...
                         (Simm13 (word_of_int simm13)),
                         (W5 (word_of_int rd))])
    else if op3 = uint(0b110011::word6) then \<comment>\<open> WRTBR \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
    ...
    \<comment>\<open> The following are arithmetic instructions. \<close>
      ...
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (arith_type ADDXcc,[(Flag (word_of_int flagi)),
                        ...
                        (Simm13 (word_of_int simm13)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b100000::word6) then \<comment>\<open> TADDcc \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (arith_type TADDcc,[(Flag (word_of_int flagi)),
                        ...
                        (Simm13 (word_of_int simm13)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b100010::word6) then \<comment>\<open> TADDccTV \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
      ...
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (arith_type SUBXcc,[(Flag (word_of_int flagi)),
                        ...
                        (Simm13 (word_of_int simm13)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b100001::word6) then \<comment>\<open> TSUBcc \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (arith_type TSUBcc,[(Flag (word_of_int flagi)),
                        ...
                        (Simm13 (word_of_int simm13)),
                        (W5 (word_of_int rd))])
    else if op3 = uint(0b100011::word6) then \<comment>\<open> TSUBccTV \<close>
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
      ...
      if flagi = 0 then \<comment>\<open> return [i,rs1,rs2,rd] \<close>
         ...
         Inr (ctrl_type RESTORE,[(Flag (word_of_int flagi)),
                           ...
                           (Simm13 (word_of_int simm13)),
                           (W5 (word_of_int rd))])
    else if op3 = uint(0b111010::word6) then \<comment>\<open> Ticc \<close>
      let trap_cond = get_trap_cond w in
      let trap_imm7 = get_trap_imm7 w in
      if trap_cond = uint(0b1000::word4) then \<comment>\<open> TA \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          Inr (ticc_type TA,[(Flag (word_of_int flagi)),
                   (W5 (word_of_int rs1)),
                   (W5 (word_of_int rs2))])
        else \<comment>\<open> return [i,rs1,trap_imm7] \<close>
          Inr (ticc_type TA,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0000::word4) then \<comment>\<open> TN \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TN,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1001::word4) then \<comment>\<open> TNE \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TNE,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0001::word4) then \<comment>\<open> TE \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TE,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1010::word4) then \<comment>\<open> TG \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TG,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0010::word4) then \<comment>\<open> TLE \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TLE,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1011::word4) then \<comment>\<open> TGE \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TGE,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0011::word4) then \<comment>\<open> TL \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TL,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1100::word4) then \<comment>\<open> TGU \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TGU,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0100::word4) then \<comment>\<open> TLEU \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TLEU,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1101::word4) then \<comment>\<open> TCC \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TCC,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0101::word4) then \<comment>\<open> TCS \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TCS,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1110::word4) then \<comment>\<open> TPOS \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TPOS,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0110::word4) then \<comment>\<open> TNEG \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TNEG,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b1111::word4) then \<comment>\<open> TVC \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
          ...
          Inr (ticc_type TVC,[(Flag (word_of_int flagi)),
                    (W5 (word_of_int rs1)),
                    (Imm7 (word_of_int trap_imm7))])
      else if trap_cond = uint(0b0111::word4) then \<comment>\<open> TVS \<close>
        if flagi = 0 then \<comment>\<open> return [i,rs1,rs2] \<close>
...
where "fetch_instruction s \<equiv> 
  \<comment>\<open> pc_val is the 32-bit memory address of the instruction. \<close>
  let pc_val = cpu_reg_val PC s;
      psr_val = cpu_reg_val PSR s;
      s_val = get_S psr_val;
      asi = if s_val = 0 then word_of_int 8 else word_of_int 9
  in
  \<comment>\<open> Check if pc_val is aligned to 4-byte (32-bit) boundary. 
     That is, check if the least significant two bits of 
     pc_val are 0s. \<close>
    if uint(bitAND (0b00000000000000000000000000000011) pc_val) = 0 then
    \<comment>\<open> Get the 32-bit value from the address of pc_val 
       to the address of pc_val+3 \<close>
    ...
    if icc_val = 1 then 
      do
        ...
        if (instr_name = bicc_type BA) \<and> (flaga = 1) then 
          ...
          return ()
      od
    else \<comment>\<open> icc_val = 0 \<close> 
      do
...
where "save_retore_sub1 result new_cwp rd \<equiv>
  ...
  write_cpu new_psr_val PSR; \<comment>\<open> Change CWP to the new window value. \<close>
  write_reg result (ucast new_cwp) rd; \<comment>\<open> Write result in rd of the new window. \<close>
  return ()
  od"
    ...
    if instr_name = ctrl_type SAVE then 
      do
        ...
        if (get_WIM_bit (unat new_cwp) wim_val) \<noteq> 0 then 
          ...
          od
      od
    else \<comment>\<open> instr_name = RESTORE \<close>
      do
        new_cwp \<leftarrow> gets (\<lambda>s. ((word_of_int (((uint curr_win) + 1) mod NWINDOWS)))::word5);
        if (get_WIM_bit (unat new_cwp) wim_val) \<noteq> 0 then 
          do
...
\<comment>\<open>    flush_cache_line(addr);\<close>
\<comment>\<open>    flush_Ibuf_and_pipeline(addr);\<close>
    return ()
  od"
    ...
    if (instr_name \<in> {sreg_type RDPSR,sreg_type RDWIM,sreg_type RDTBR} \<or> 
      ...
      if instr_name = sreg_type RDY then
        ...
        do
          ...
          return ()
        od
      else \<comment>\<open> Must be RDTBR. \<close>
        do
  ...
  do
    ...
    if instr_name = sreg_type WRY then
      ...
      if s_val = 0 then
        ...
        do \<comment>\<open> ET and PIL appear to be written IMMEDIATELY w.r.t. interrupts. \<close>
      ...
      if s_val = 0 then
        ...
        do \<comment>\<open> Don't write bits corresponding to non-existent windows. \<close>
          ...
          return ()
        od
    else  \<comment>\<open> Must be WRTBR \<close>
      if s_val = 0 then
        do
          raise_trap privileged_instruction;
          return ()
        ...
        do \<comment>\<open> Only write the bits <31:12> of the result to TBR. \<close>
          tbr_val \<leftarrow> gets (\<lambda>s. (cpu_reg_val TBR s));
...
where "mul_instr instr \<equiv>
  ...
  do
    ...
    \<comment>\<open> whether to use ucast or scast does not matter below. \<close>
...
where "div_comp instr rs1 rd operand2 \<equiv>
  do
    ...
    \<comment>\<open>result \<leftarrow>  gets (\<lambda>s. ((ucast temp_64bit)::word32));\<close>
...
where "ld_asi instr s_val \<equiv>
  ...
  else \<comment>\<open> Must be LDA, LDUBA, LDSBA, LDSHA, LDUHA, or LDDA. \<close>
  ...
  do
    ...
    do
      ...
      do
        ...
        return ()
      od
      else \<comment>\<open> Must be LDD or LDDA \<close>
...
where "load_instr instr \<equiv>
  let instr_name = fst instr;
      ...
      rd = if instr_name \<in> {load_store_type LDUBA,load_store_type LDA,
        ...
        load_store_type LDSHA,load_store_type LDDA} then \<comment>\<open> rd is member 4 \<close>
            get_operand_w5 (op_list!4)
           else \<comment>\<open> rd is member 3\<close>      
...
where "st_asi instr s_val \<equiv>
  ...
  else \<comment>\<open> Must be STA, STBA, STHA, STDA. \<close>
    get_operand_asi ((snd instr)!3)
"
...
where "st_byte_mask instr address \<equiv>
  if (fst instr) \<in> {load_store_type STD,load_store_type ST,
    ...
    if ((ucast address)::word2) = 0 then
      ...
      (0b0011::word4)
  else \<comment>\<open> Must be STB or STBA. \<close>
    if ((ucast address)::word2) = 0 then 
      (0b1000::word4)
...
where "store_instr instr \<equiv>
  let instr_name = fst instr;
      ...
      rd = if instr_name \<in> {load_store_type STA,load_store_type STBA,
            load_store_type STHA,load_store_type STDA} then \<comment>\<open> rd is member 4 \<close>
            get_operand_w5 (op_list!4)
           else \<comment>\<open> rd is member 3\<close>      
            get_operand_w5 (op_list!3)
  in
...
where "ldst_asi instr s_val \<equiv>
  ...
  else \<comment>\<open> Must be LDSTUBA. \<close>
    get_operand_asi ((snd instr)!3)
"
...
where "load_store_sub1 instr rd s_val \<equiv>
  do
    ...
    \<comment>\<open> an implementation actually need only block when another LDSTUB or SWAP
      is pending on the same byte in memory as the one addressed by this LDSTUB\<close>
    \<comment>\<open> Should wait when block_type = 1 \<or> block_word = 1 \<close>
    \<comment>\<open> until another processes write both to be 0. \<close>
    \<comment>\<open> We implement this as setting pc as npc when the instruction 
      is blocked. This way, in the next iteration, we will still execution
      the current instruction. \<close>
...
where "load_store_instr instr \<equiv>
  let instr_name = fst instr;
      op_list = snd instr;
      flagi = get_operand_flag (op_list!0);
      rd = if instr_name \<in> {load_store_type LDSTUBA} then \<comment>\<open> rd is member 4 \<close>
            get_operand_w5 (op_list!4)
           else \<comment>\<open> rd is member 3\<close>      
            get_operand_w5 (op_list!3)
  in
...
where "swap_instr instr \<equiv>
  let instr_name = fst instr;
      op_list = snd instr;
      flagi = get_operand_flag (op_list!0);
      rd = if instr_name \<in> {load_store_type SWAPA} then \<comment>\<open> rd is member 4 \<close>
            get_operand_w5 (op_list!4)
           else \<comment>\<open> rd is member 3\<close>      
            get_operand_w5 (op_list!3)
  in
...
text {* The following three definitions are for the CASA and CASXA instructions 
...
definition comp_swap_sub2 :: "virtua_address \<Rightarrow> asi_type \<Rightarrow> word5 \<Rightarrow> word5 \<Rightarrow>
  ('a::len0) window_size \<Rightarrow> word32 \<Rightarrow> ('a,unit) sparc_state_monad"
where "comp_swap_sub2 address asi rs2 rd curr_win word0 \<equiv>
  do        
    (result1,new_state1) \<leftarrow> gets (\<lambda>s. (memory_read asi (address + 4) s));
    if result1 = None then
    do
    ...
    do
      word1 \<leftarrow> gets (\<lambda>s. (case result1 of Some v \<Rightarrow> v));
      modify (\<lambda>s. (new_state1));
      rs2_val0 \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win (bitAND rs2 0b11110) s));
      rs2_val1 \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win (bitOR rs2 0b00001) s));
      rd_val0 \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win (bitAND rd 0b11110) s));
      rd_val1 \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win (bitOR rd 0b00001) s));
      if word0 = rs2_val0 \<and> word1 = rs2_val1 then \<comment>\<open> swap the values in address and rd \<close>
      do
        byte_mask \<leftarrow> gets (\<lambda>s. (0b1111::word4));
        result0 \<leftarrow> gets (\<lambda>s. (memory_write asi address byte_mask rd_val0 s));        
        if result0 = None then 
        do
          raise_trap data_access_exception;
          return ()
        ...
        do
          new_state0 \<leftarrow> gets (\<lambda>s. (case result0 of Some v \<Rightarrow> v));
          modify (\<lambda>s. (new_state0));
          result1 \<leftarrow> gets (\<lambda>s. (memory_write asi (address + 4) byte_mask rd_val1 s));
          if result1 = None then 
          do
            raise_trap data_access_exception;
            return ()
          ...
          do
            new_state1 \<leftarrow> gets (\<lambda>s. (case result1 of Some v \<Rightarrow> v));
            modify (\<lambda>s. (new_state1));
            write_reg word1 curr_win (bitOR rd 0b00001);
            if rd \<noteq> 0 then
            do
              write_reg word0 curr_win (bitAND rd 0b11110);
              return ()
        ...
        od
      od  
      else \<comment>\<open> only write the values in address to rd \<close>
      do
        write_reg word1 curr_win (bitOR rd 0b00001);
        if rd \<noteq> 0 then
        do
...
definition comp_swap_sub1 :: "instruction \<Rightarrow> word5 \<Rightarrow> word5 \<Rightarrow> word5 \<Rightarrow> word1 \<Rightarrow> word8 \<Rightarrow> 
  ('a::len0,unit) sparc_state_monad"
  where "comp_swap_sub1 instr rs1 rs2 rd s_val asi \<equiv>
  do
    curr_win \<leftarrow> get_curr_win();
    address \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win rs1 s));    
    if ((fst instr) = load_store_type CASXA \<and> ((ucast address)::word3) \<noteq> 0) 
         \<or> ((fst instr) = load_store_type CASA \<and> ((ucast address)::word2) \<noteq> 0)
    then
    do
      raise_trap mem_address_not_aligned;
      return ()
    ...
    do      
      \<comment>\<open> SPARCv9 manual says CASA and CASXA don't involve memory barrier semantics. \<close>
      (result,new_state) \<leftarrow> gets (\<lambda>s. (memory_read asi address s));
      if result = None then
      do
        raise_trap data_access_exception;
      ...
      do
        word \<leftarrow> gets (\<lambda>s. (case result of Some v \<Rightarrow> v));
        modify (\<lambda>s. (new_state));
        rs2_val \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win rs2 s));
        rd_val \<leftarrow> gets (\<lambda>s. (user_reg_val curr_win rd s)); 
        if (fst instr) = load_store_type CASA then
          if word = rs2_val then \<comment>\<open> swap the value in memory address and rd \<close>
          do
            byte_mask \<leftarrow> gets (\<lambda>s. (0b1111::word4));
            result0 \<leftarrow> gets (\<lambda>s. (memory_write asi address byte_mask rd_val s));
            if result0 = None then
            do
              raise_trap data_access_exception;
              return ()
            ...
            od
          od
          else \<comment>\<open> only write the value in memory address to rd \<close>
            if rd \<noteq> 0 then 
            do
              write_reg word curr_win rd;
              return ()
            od
              ...
              return () 
        else \<comment>\<open> CASXA \<close>
          comp_swap_sub2 address asi rs2 rd curr_win word 
      od
    od
  od"  
  
text {* Operational semantics for Compare and Swap. *}
definition comp_swap_instr :: "instruction \<Rightarrow> ('a::len0,unit) sparc_state_monad"
where "comp_swap_instr instr \<equiv>
  let instr_name = fst instr;
      op_list = snd instr;
      flagi = get_operand_flag (op_list!0);
      rs1 = get_operand_w5 ((snd instr)!1);
      rs2 = get_operand_w5 ((snd instr)!2);
  ...
  do
    ...
    s_val \<leftarrow> gets (\<lambda>s. (get_S psr_val));
    asi \<leftarrow> gets (\<lambda>s. (ldst_asi instr s_val)); 
    if asi \<noteq> 10 \<and> s_val = 0 then \<comment>\<open> We require that if the instruction is executed in user mode, the ASI must be 10/0xA (user data). \<close>
      do
        raise_trap privileged_instruction;
        return ()
      od
    else if flagi = 1 then \<comment>\<open> SPARCv9 uses ASI register when i = 1. There are no ASI registers 
    for SPARCv8 processors, so we forbid this case. \<close>
      do
        raise_trap illegal_instruction;
        return ()
      od
    else
      comp_swap_sub1 instr rs1 rs2 rd s_val asi
  od"  
