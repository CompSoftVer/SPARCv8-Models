...
text {* SPARC V8 architecture CPU model*}
theory Sparc_Types    
...
type_synonym word_length_t1 = 8
type_synonym word_length_t2 = 6
type_synonym word_length_t3 = 6
type_synonym word_length_offset = 12
type_synonym word_length_page = 24
type_synonym word_length_phys_address = 36
type_synonym word_length_virtua_address = 32
type_synonym word_length_entry_type = 2
type_synonym word_length_machine_word = 32
...
datatype CPU_register = 
 ...
 \<comment>\<open>| CCR    \<comment> \<open>   "Cache Control Register"\<close>\<close>
...
where
"get_S psr \<equiv> 
\<comment>\<open>  ucast ((bitAND psr 0b00000000000000000000000010000000) >> 7) \<close>
...
datatype Exception =
\<comment>\<open> The following are processor states that are not in the instruction model,
...
invalid_cond_f2
|invalid_op2_f2
|illegal_instruction2 \<comment>\<open> when i = 0 for load/store not from alternate space \<close>
|invalid_op3_f3_op11
|case_impossible
|invalid_op3_f3_op10
|invalid_op_f3
