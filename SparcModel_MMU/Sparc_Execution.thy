...
where "select_trap _ \<equiv>
  do
    ...
    if rt_val = True then \<comment>\<open> ignore ET, and leave tt unchaged \<close>
      return ()
    ...
    \<comment>\<open> By the SPARCv8 manual only 1 of the following traps could be in traps. \<close>
      ...
      do 
        ...
        return ()
      od
    \<comment>\<open>else if interrupt_level > 0 then \<close>
    \<comment>\<open> We don't consider interrupt_level \<close>
    else return ()
  od"
  ...
  do
    ...
    if annul = False then
      do
        ...
        return ()
      od
    else \<comment>\<open> annul = True \<close> 
      do
  ...
  do
    ...
    if reset_trap = False then
      do
        ...
        return ()
      od
    else \<comment>\<open> reset_trap = True \<close>
      do
        write_cpu 0 PC;
        write_cpu 4 nPC;
  ...
  do
    ...
    if traps = {} then 
      if instr_name \<in> {load_store_type LDSB,load_store_type LDUB,
        ...
        branch_instr instr
      else if instr_name \<in> {load_store_type CASA, load_store_type CASXA} then
        comp_swap_instr instr
      else fail ()
    else return ()
  od"
  ...
  do
    ...
    do
      ...
      do
          ...
          case dec of 
            ...
            | Inr v2 \<Rightarrow> (do
              ...
              do
                ...
                return ()
              od
              else \<comment>\<open> annul \<noteq> False \<close>
              do
                set_annul False;
                npc_val \<leftarrow> gets (\<lambda>s. (cpu_reg_val nPC s));
                write_cpu npc_val PC;
        ...
        od) 
      od
      else return () \<comment>\<open> Not in execute_mode. \<close>
    od
    else \<comment>\<open> traps is not empty, which means trap = 1. \<close>
    do
      execute_trap();
      return ()
    od
...
where "good_context s \<equiv> 
  ...
  in
  if traps \<noteq> {} \<and> rt_val = False \<and> et_val = 0 then False \<comment>\<open> enter error_mode in select_traps. \<close>
  else
    let s' = delayed_pool_write s in
    case fetch_instruction s' of
    \<comment>\<open> instruction_access_exception is handled in the next state. \<close>
    ...
    |Inr v \<Rightarrow> (
      ...
      |Inr instr \<Rightarrow> (
        ...
        else \<comment>\<open> annul = False \<close>
          if supported_instruction (fst instr) then
            \<comment>\<open> The only instruction that could fail is RETT. \<close>
...
type_synonym leon3_state = "(5) sparc_state"
