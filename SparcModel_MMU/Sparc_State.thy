...
"user_reg_mod data_w32 win ur state \<equiv>
  if ur = 0 then state
    ...
    state\<lparr>user_reg := 
      (user_reg state)(win := ((user_reg state) win)(ur := data_w32))\<rparr>
  else \<comment>\<open> if 23 < ur \<and> ur < 32 then \<close>
    in_reg_mod data_w32 win ur state
\<comment>\<open>  else state \<close>
"
...
definition mem_val_alt_89:: "asi_type \<Rightarrow> phys_address \<Rightarrow> 
                      ('a) sparc_state \<Rightarrow> mem_val_type option"
where
"mem_val_alt_89 asi add state \<equiv> 
  let r = (mem state) asi add in
  if r = None \<and> (uint asi) = 8 then (mem state) (word_of_int 9) add
  else if r = None \<and> (uint asi) = 9 then (mem state) (word_of_int 8) add
  else r"

definition mem_val_alt_1011:: "asi_type \<Rightarrow> phys_address \<Rightarrow> 
                      ('a) sparc_state \<Rightarrow> mem_val_type option"
where
"mem_val_alt_1011 asi add state \<equiv> 
  let r = (mem state) asi add;
      r10 = (mem state) (word_of_int 10) add;
      r11 = (mem state) (word_of_int 11) add 
  in
  if r = None \<and> (uint asi) = 10 then 
    if r11 = None then mem_val_alt_89 8 add state
    else r11
  else if r = None \<and> (uint asi) = 11 then
    if r10 = None then mem_val_alt_89 8 add state
    else r10
  else r"
...
"mem_val_alt asi add state \<equiv> 
  if (uint asi) \<in> {8,9} then mem_val_alt_89 asi add state
  else mem_val_alt_1011 asi add state"
...
"mem_mod asi addr val state \<equiv>
  ...
  in \<comment>\<open> Only allow one of asi 8 and 9 (10 and 11) to have value. \<close>
...
"mem_mod_alt asi addr val state \<equiv>
  let state1 = state\<lparr>mem := (mem state)
    ...
    asi11 = word_of_int 11
  in 
  \<comment>\<open> Only allow one of asi 8, 9, 10, 11 to have value. \<close>
  if (uint asi) = 8 then 
...
"ccr_flush state \<equiv>
  let ccr_val = sys_reg_val CCR state;
      \<comment>\<open> FI is bit 21 of CCR \<close>
...
where "memory_read asi addr state \<equiv>
  ...
  if asi_int = 1 then \<comment>\<open> Forced cache miss. \<close>
    ...
    \<comment>\<open> We don't go through the tradition, i.e., read from cache first,
      if the address is not cached, then read from memory, 
      because performance is not an issue here. 
      Thus we directly read from memory and update the cache. \<close>
    ...
    \<comment>\<open> We don't go through the tradition, i.e., read from cache first,
      ...
      ((load_word_mem state addr asi),state)
  \<comment>\<open> We don't access instruction cache tag. i.e., asi = 12. \<close>    
  ...
  else if asi_int = 24 then \<comment>\<open> Flush cache and TLB in LEON3. 
    But is not used for memory read. \<close>
    (None, state)
  ...
  else if asi_int = 28 then \<comment>\<open> MMU bypass. 
    Directly use addr as a physical address. 
    Append 0000 in the front of addr. 
    In this case, (ucast addr) suffices. \<close>
...
"memory_write_asi asi addr byte_mask data_w32 state \<equiv> 
  ...
  if asi_int = 1 then \<comment>\<open> Forced cache miss. \<close>
    ...
    \<comment>\<open> Assuming writing into asi = 10. \<close>
    store_word_mem state addr data_w32 byte_mask (word_of_int 10) 
  else if asi_int = 2 then \<comment>\<open> System registers. \<close>
    \<comment>\<open> See Table 19, Page 34 for System Register address map in LEON 3FT. \<close>
    if uint addr = 0 then \<comment>\<open> Cache control register. \<close>      
      let s1 = (sys_reg_mod data_w32 CCR state) in
      \<comment>\<open> Flush the instruction cache if FI of CCR is 1;
         flush the data cache if FD of CCR is 1. \<close>
      Some (ccr_flush s1)
    else if uint addr = 8 then \<comment>\<open> Instruction cache configuration register. \<close>
  ...
  \<comment>\<open> We don't access data cache tag. i.e., asi = 14. \<close>
    ...
    \<comment>\<open> Write to virtual address as physical address. 
      Append 0000 in front of addr. \<close>
